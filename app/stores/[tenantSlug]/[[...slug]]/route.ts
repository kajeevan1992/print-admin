...
const KNOWN_THEME_SOURCES: Record<string, string> = {
...
function rewriteJavaScript(js: string) {
  return js
    .replace(/window\.location\.pathname/g, '(window.__PRINT_ADMIN_THEME_PATH__||window.location.pathname)')
    .replace(/(?<!window\.)location\.pathname/g, '(window.__PRINT_ADMIN_THEME_PATH__||location.pathname)');
}

function rewriteCss(css: string, storeBase: string) {
  return css
    .replace(/url\(\/images\//g, `url(${storeBase}/__theme-assets/images/`)
    .replace(/url\(\/assets\//g, `url(${storeBase}/__theme-assets/assets/`);
}
...
export async function GET(request: Request, { params }: Params) {
  ...
  if (isAsset) {
    if (finalType.includes('javascript') || sourcePath.endsWith('.js')) {
      const js = rewriteJavaScript(await upstream.text());
      return new NextResponse(js, { status: upstream.status, headers: { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'no-store' } });
    }
    if (finalType.includes('text/css') || sourcePath.endsWith('.css')) {
      const css = rewriteCss(await upstream.text(), storeBase);
      return new NextResponse(css, { status: upstream.status, headers: { 'content-type': 'text/css; charset=utf-8', 'cache-control': 'no-store' } });
    }
    const body = await upstream.arrayBuffer();
    return new NextResponse(body, { status: upstream.status, headers: { 'content-type': finalType, 'cache-control': 'public, max-age=300' } });
  }

  const html = await upstream.text();
  if (!finalType.includes('text/html')) {
    return new NextResponse(html, { status: upstream.status, headers: { 'content-type': finalType, 'cache-control': 'no-store' } });
  }

  return new NextResponse(rewriteHtml(html, sourceBase, storeBase, themePath, cleanTenantSlug, store.tenantId || ids[0] || cleanTenantSlug, storeSlug), {
