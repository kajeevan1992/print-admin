...
import { AlertTriangle, CheckCircle2, Clock3, CreditCard, ExternalLink, PackageCheck, RefreshCcw, Search, ShieldCheck, Truck, UploadCloud } from 'lucide-react';
...
function eventTone(action: string) { if (action === 'approved') return 'border-emerald-400/25 bg-emerald-400/10 text-emerald-100'; if (action === 'revision-requested') return 'border-amber-400/25 bg-amber-400/10 text-amber-100'; if (action === 'proof-sent') return 'border-sky-400/25 bg-sky-400/10 text-sky-100'; return 'border-white/10 bg-white/[0.04] text-slate-300'; }
function eventTitle(event: any) { if (event?.proofVersion) return `${event.label || 'Proof event'} · v${event.proofVersion}`; return event?.label || 'Proof event'; }
...
function ProofEventsCard({ events }: { events?: any[] }) {
  const list = Array.isArray(events) ? events : [];
  if (!list.length) return null;
  return <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
    <div className="flex items-center gap-2 text-sky-100"><Clock3 size={18}/><h3 className="font-black">Proof history</h3></div>
    <p className="mt-2 text-sm leading-6 text-slate-400">Latest proof rounds and customer decisions for this order.</p>
    <div className="mt-4 space-y-3">
      {list.map((event) => <div key={event.id || `${event.action}-${event.at}-${event.proofVersion}`} className={`rounded-2xl border p-4 ${eventTone(event.action)}`}>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-sm font-black text-white">{eventTitle(event)}</p><p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">{event.actor === 'customer' ? 'Customer action' : 'Store update'}</p></div>
          <p className="text-xs font-bold text-slate-300">{dateLabel(event.at)}</p>
        </div>
        {event.note ? <p className="mt-3 text-sm leading-6 text-slate-300">{event.note}</p> : null}
        <div className="mt-3 flex flex-wrap gap-2">
          {event.productionReleaseState ? <span className="rounded-full border border-white/10 bg-slate-950/50 px-3 py-1 text-xs font-bold text-slate-300">Production: {event.productionReleaseState}</span> : null}
          {event.emailStatus ? <span className="rounded-full border border-white/10 bg-slate-950/50 px-3 py-1 text-xs font-bold text-slate-300">Email: {event.emailStatus}</span> : null}
        </div>
      </div>)}
    </div>
  </section>;
...
export default function TrackOrderPage() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); const id = params.get('orderId') || params.get('orderNumber') || ''; const mail = params.get('email') || ''; if (id) setOrderId(id); if (mail) setEmail(mail); if (id) void lookup(id, mail); }, []);
  async function lookup(id = orderId, mail = email) { setLoading(true); setError(''); setStatus(null); try { const params = new URLSearchParams({ orderId: id }); if (mail) params.set('email', mail); const res = await fetch(`/api/native-storefront/order-status?${params.toString()}`, { cache: 'no-store' }); const json = await res.json().catch(() => ({})); if (!res.ok || json.ok === false) throw new Error(json.error || 'Order status failed'); setStatus(json.data); } catch (err) { setError(err instanceof Error ? err.message : 'Order status failed'); } finally { setLoading(false); } }
  const active = useMemo(() => status?.progress?.find((step) => step.state === 'active'), [status]);
  const paid = paymentReady(status);
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl space-y-6"><section className="rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.18),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(2,6,23,0.96))] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.25)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.25em] text-sky-200">Customer order tracking</p><h1 className="mt-2 text-4xl font-black tracking-[-0.06em]">Track your print order</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">See artwork checks, proof approval, production, and dispatch progress from the same backend workflow used by the print team.</p></div><div className="rounded-2xl border border-sky-400/20 bg-sky-400/10 px-4 py-3 text-sm font-bold text-sky-100">Live production status</div></div></section>
    <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-4"><div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"><input value={orderId} onChange={(event) => setOrderId(event.target.value)} placeholder="Order number or order ID" className="rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-sky-400/30 placeholder:text-slate-500 focus:ring-2" /><input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email address optional" className="rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-sky-400/30 placeholder:text-slate-500 focus:ring-2" /><button onClick={() => void lookup()} disabled={loading || !orderId.trim()} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{loading ? <RefreshCcw size={16} className="animate-spin" /> : <Search size={16} />} Check status</button></div></section>
    ...
    {status ? <><NextActionCard action={status.nextAction} /><section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-slate-500">{status.order?.orderNumber}</p><h2 className="mt-2 text-2xl font-black">{status.message}</h2><p className="mt-2 text-sm text-slate-400">Current stage: {active?.label || status.currentStage}</p><div className="mt-3 flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1 text-xs font-bold ${paid ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-100' : 'border-amber-400/30 bg-amber-400/10 text-amber-100'}`}>Payment: {status.order?.paymentStatus || status.artwork?.paymentStatus || 'unpaid'}</span><span className={`rounded-full border px-3 py-1 text-xs font-bold ${status.artwork?.paymentReleased ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-100' : 'border-white/10 bg-white/[0.04] text-slate-300'}`}>Proof/payment gate: {status.artwork?.paymentReleased ? 'released' : status.artwork?.paymentGate || 'checking'}</span>{status.artwork?.proofVersion ? <span className="rounded-full border border-sky-400/30 bg-sky-400/10 px-3 py-1 text-xs font-bold text-sky-100">Proof v{status.artwork.proofVersion}</span> : null}</div></div><div className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-right"><p className="text-xs text-slate-500">Order total</p><p className="mt-1 text-lg font-black">{money(status.order?.total, status.order?.currency)}</p></div></div><div className="mt-6 grid gap-3 md:grid-cols-3 xl:grid-cols-6">{(status.progress || []).map((step: Step) => <div key={step.key} className={`rounded-2xl border p-3 ${tone(step.state)}`}><div className="flex items-center gap-2"><CheckCircle2 size={15}/><p className="text-xs font-black uppercase tracking-[0.12em]">{step.state}</p></div><p className="mt-2 text-sm font-bold">{step.label}</p></div>)}</div></div><div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><h3 className="text-lg font-black">Order items</h3><div className="mt-3 space-y-2">{(status.order?.items || []).map((item: any, index: number) => <div key={`${item.productName}-${index}`} className="rounded-2xl border border-white/10 bg-slate-950/60 p-3 text-sm"><div className="flex justify-between gap-3"><span className="font-bold">{item.productName}</span><span className="text-slate-400">× {item.quantity}</span></div></div>)}</div></div></section><section className="grid gap-4 md:grid-cols-3"><InfoCard icon={ShieldCheck} title="Artwork / Proof" value={status.artwork?.customerProofStatus || status.artwork?.artworkStatus || 'Not started'} text={status.artwork?.blockReason || (status.artwork?.preflightStatus ? `Preflight: ${status.artwork.preflightStatus}` : 'Artwork will update here once uploaded.')} /><InfoCard icon={Clock3} title="Production" value={status.production?.stage || 'Not scheduled'} text={status.production?.blockReason || `Scheduled: ${dateLabel(status.production?.scheduledStartAt)}`} /><InfoCard icon={Truck} title="Dispatch" value={status.dispatch?.stage || 'Not ready'} text={status.dispatch?.trackingNumber ? `Tracking: ${status.dispatch.trackingNumber}` : 'Dispatch details will appear once packed.'} /></section><ProofEventsCard events={status.proofEvents} /></> : <section className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] p-10 text-center text-slate-400"><PackageCheck className="mx-auto mb-3" size={34}/><p>Enter an order number to see live artwork, production, and dispatch progress.</p></section>}
  </div></main>;
}
function InfoCard({ icon: Icon, title, value, text }: any) { return <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2 text-sky-100"><Icon size={18}/><h3 className="font-black">{title}</h3></div><p className="mt-4 text-xl font-black">{value}</p><p className="mt-2 text-sm leading-6 text-slate-400">{text}</p></div>; }
