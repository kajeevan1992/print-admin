...
function footerStats(value: unknown) {
  if (Array.isArray(value)) return value.slice(0, 4).map((item: any) => [String(item?.label || item?.title || ''), String(item?.value || item?.count || '')] as [string, string]).filter(([label, amount]) => label || amount);
  return String(value || '').split(/\r?\n/g).map((line) => {
    const [label, ...rest] = line.split('|');
    return [String(label || '').trim(), rest.join('|').trim()] as [string, string];
  }).filter(([label, amount]) => label || amount).slice(0, 4);
}

function safeNewsletterAction(value: unknown) {
  const action = String(value || '').trim();
  if (!action) return '';
  return action.startsWith('/') || /^https:\/\//i.test(action) ? action : '';
}
...
export default function ChromeFooter({ storeBase, settings, navItems, logo, studio = false }: { storeBase: string; settings: StorefrontRuntimeSettings; navItems: NavItem[]; logo: ReactNode; studio?: boolean }) {
  ...
  const configuredStats = footerStats(content.footerStats);
  const stats = configuredStats.length ? configuredStats : [['Business printing', '20+'], ['Event signage', '12+'], ['Labels & packaging', '18+'], ['Custom quote support', '1:1']];
  const columns = columnsFromNavigation(navItems);
  const newsletterAction = safeNewsletterAction(content.newsletterAction);
