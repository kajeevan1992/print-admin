...
export async function renderAtlantisStorefront(context: StorefrontRuntimeContext) {
  const { tenantSlug, storeSlug, storeBase, navItems, products, routeSegments, tenantIds, collectionPoints } = context;
  ...
  if (routeSegments[0] === 'cart') return <CartPage storeBase={storeBase} navItems={navItems} products={products} />;
