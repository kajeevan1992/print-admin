...
export const ownerQbrsService = {
  async list() {
    await wait();
    return readStore<OwnerQbrRecord>(STORAGE_KEY, ownerQbrSeed);
  },
  async save(record: OwnerQbrRecord) {
    await wait();
    const items = readStore<OwnerQbrRecord>(STORAGE_KEY, ownerQbrSeed);
  ...
  async delete(id: string) {
    await wait();
    writeStore(STORAGE_KEY, readStore<OwnerQbrRecord>(STORAGE_KEY, ownerQbrSeed).filter((item) => item.id !== id));
  },
  async reset() {
    await wait();
    writeStore(STORAGE_KEY, ownerQbrSeed);
    return ownerQbrSeed;
  }
};
