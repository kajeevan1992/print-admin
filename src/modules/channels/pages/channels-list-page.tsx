...
export function ChannelsListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const map = Object.fromEntries(
        themesResponse.data.items.map((theme) => [theme.id, theme.name])
      );
      setThemes(map);
  ...
  return (
    <div>
      <PageHeader
        title="Channels"
        subtitle="Manage storefront channels, domains, and frontend delivery modes."
        actions={<PrimaryButton onClick={() => setOpen(true)}>+ Create Channel</PrimaryButton>}
      />
      ...
      {loading ? (
        <div className="rounded-xl border border-border bg-panel p-6 text-sm">Loading channels...</div>
      ) : null}
      {error ? (
      ...
      {!loading && !error && channels.length === 0 ? (
        <EmptyModuleState
          ...
          description="Create your first storefront channel to start assigning themes and products."
        />
      ) : null}
        ...
        <DataTable
          columns={[
            ...
            { key: 'theme', header: 'Theme', render: (row) => themes[row.themeId] ?? row.themeId },
            {
              key: 'type',
              header: 'Type',
              render: (row) => (row.isHeadless ? 'Headless' : 'Hosted')
            },
            {
              key: 'actions',
              header: 'Actions',
              render: (row) => (
                <div className="flex gap-2">
                  <Link href={`/channels/${row.id}`} className="text-accent">
                    Open
                  </Link>
                  <Button>Edit</Button>
                </div>
