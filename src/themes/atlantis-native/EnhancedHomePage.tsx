...
function PromoBanner({ section, storeBase }: { section: StorefrontHomepageSection; storeBase: string }) {
  const eyebrow = text(section.eyebrow);
  const title = text(section.title);
  ...
  return <section className="py-8"><Shell><div className="relative min-h-[300px] overflow-hidden rounded-[30px] bg-slate-950 p-8 text-white md:p-12">{image ? <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-55" /> : null}<div className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/55 to-black/15" /><div className="relative max-w-[720px]">{eyebrow ? <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/65">{eyebrow}</div> : null}{title ? <h2 className="mt-3 text-[40px] font-black leading-[0.96] tracking-[-0.06em] md:text-[52px]">{title}</h2> : null}{body ? <p className="mt-4 max-w-[640px] text-[13px] leading-7 text-white/78">{body}</p> : null}{buttonLabel && buttonPath ? <div className="mt-6"><Link href={storeHref(storeBase, buttonPath)} className="inline-flex rounded-full bg-white px-5 py-3 text-[12px] font-black text-black no-underline">{buttonLabel}</Link></div> : null}</div></div></Shell></section>;
}
...
function HostedSection({ section, storeBase, products, categories, collectionPoints }: { section: StorefrontHomepageSection; storeBase: string; products: ThemeProductCard[]; categories: ThemeCategoryCard[]; collectionPoints: CollectionPoint[] }) {
  ...
  if (section.type === 'promo-banner') return <PromoBanner section={section} storeBase={storeBase} />;
  ...
  if (section.type === 'category-carousel') {
    ...
    return <section className="py-8"><Shell><SectionHeading eyebrow={text(section.eyebrow)} title={text(section.title)} body={text(section.subtitle || section.body)} />{shown.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{shown.map((category) => <Link key={category.slug} href={storeHref(storeBase, `/${category.slug}`)} className="overflow-hidden rounded-[22px] border bg-white no-underline shadow-sm" style={{ borderColor: BRAND.line }}>{category.image ? <img src={category.image} alt={category.title} className="h-36 w-full object-cover" /> : null}<div className="p-5"><div className="text-[18px] font-black" style={{ color: BRAND.ink }}>{category.title}</div>{category.description ? <p className="mt-2 text-[12px] leading-6" style={{ color: BRAND.muted }}>{category.description}</p> : null}<div className="mt-3 text-[12px] font-black" style={{ color: BRAND.primary }}>{category.productCount} products</div></div></Link>)}</div> : <div className="rounded-[24px] border bg-white p-6 text-sm" style={{ borderColor: BRAND.line, color: BRAND.muted }}>No published categories match this section.</div>}</Shell></section>;
  }
  ...
  if (section.type === 'card-grid') {
    ...
    return <section className="py-8"><Shell><SectionHeading eyebrow={text(section.eyebrow)} title={text(section.title)} body={text(section.subtitle || section.body)} /><div className={`grid gap-4 sm:grid-cols-2 ${grid}`}>{cards.map((item: any, index) => { const image = text(item.imageUrl || item.image); const title = text(item.title); const body = text(item.body); const label = text(item.buttonLabel); const href = text(item.buttonHref); return <div key={`${title}-${index}`} className="overflow-hidden rounded-[22px] border bg-white shadow-sm" style={{ borderColor: BRAND.line }}>{image ? <img src={image} alt={title} className="h-44 w-full object-cover" /> : null}<div className="p-5">{title ? <h3 className="text-[18px] font-black" style={{ color: BRAND.ink }}>{title}</h3> : null}{body ? <p className="mt-2 text-[12px] leading-6" style={{ color: BRAND.muted }}>{body}</p> : null}{label && href ? <Link href={storeHref(storeBase, href)} className="mt-4 inline-flex text-[12px] font-black no-underline" style={{ color: BRAND.primary }}>{label} →</Link> : null}</div></div>; })}</div></Shell></section>;
  }

  if (section.type === 'testimonials') {
    const items = list(section.items).filter((item: any) => text(item?.quote));
    if (!items.length) return null;
    return <section className="py-8"><Shell><SectionHeading eyebrow={text(section.eyebrow)} title={text(section.title)} body={text(section.subtitle || section.body)} /><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{items.map((item: any, index) => <figure key={`${text(item.name)}-${index}`} className="rounded-[22px] border bg-white p-6 shadow-sm" style={{ borderColor: BRAND.line }}><Quote className="h-6 w-6" style={{ color: BRAND.primary }} /><blockquote className="mt-4 text-[14px] leading-7" style={{ color: BRAND.ink }}>“{text(item.quote)}”</blockquote><figcaption className="mt-5 flex items-center gap-3">{text(item.imageUrl) ? <img src={text(item.imageUrl)} alt="" className="h-11 w-11 rounded-full object-cover" /> : null}<div><div className="text-[12px] font-black" style={{ color: BRAND.ink }}>{text(item.name) || 'Customer'}</div>{text(item.role) ? <div className="mt-1 text-[11px]" style={{ color: BRAND.muted }}>{text(item.role)}</div> : null}</div></figcaption></figure>)}</div></Shell></section>;
  }

  if (section.type === 'trust-badges') {
    const items = list(section.items).filter((item: any) => text(item?.title) || text(item?.body));
    if (!items.length) return null;
    return <section className="py-6"><Shell>{text(section.title) || text(section.body) ? <SectionHeading eyebrow={text(section.eyebrow)} title={text(section.title)} body={text(section.subtitle || section.body)} /> : null}<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{items.map((item: any, index) => <div key={`${text(item.title)}-${index}`} className="rounded-[18px] border bg-white p-5" style={{ borderColor: BRAND.line }}>{text(item.imageUrl) ? <img src={text(item.imageUrl)} alt="" className="h-9 w-9 object-contain" /> : <CheckCircle2 className="h-6 w-6" style={{ color: BRAND.primary }} />}<div className="mt-3 text-[14px] font-black" style={{ color: BRAND.ink }}>{text(item.title)}</div>{text(item.body) ? <p className="mt-2 text-[11px] leading-5" style={{ color: BRAND.muted }}>{text(item.body)}</p> : null}</div>)}</div></Shell></section>;
  }
