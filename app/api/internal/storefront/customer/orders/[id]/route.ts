...
function taxSummary(order: any) { const netMinor = Number(order.subtotalMinor || 0); const vatMinor = Number(order.taxMinor || 0); const deliveryMinor = Number(order.shippingMinor || 0); const grossMinor = Number(order.totalMinor || Math.round(Number(order.total || 0) * 100)); return { currency: order.currency || 'GBP', netMinor, vatMinor, deliveryMinor, grossMinor, net: minorToMoney(netMinor), vat: minorToMoney(vatMinor), delivery: minorToMoney(deliveryMinor), gross: minorToMoney(grossMinor), vatBreakdown: Array.isArray(order.vatBreakdown) ? order.vatBreakdown : [], taxEnforcedAt: order.taxEnforcedAt || '' }; }
...
function normalise(order: any, uploads: any[], jobs: any[]) { const artwork = artworkSummary(order, uploads); const production = productionSummary(order, jobs); const delivery = deliverySummary(order, production); const tax = taxSummary(order); return { ...order, created_at: order.createdAt, total: tax.gross || money(order.total), artwork_status: artwork.status, delivery: delivery.method, customer: { name: order.customerName, email: order.customerEmail }, artwork, production, deliveryStatus: delivery, taxSummary: tax }; }
