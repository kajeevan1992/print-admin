...
export const categoriesService = {
  ...
  updateCategory: async (id: string, values: CategoryFormValues): Promise<ApiResponse<Category>> => {
    const category = buildCategory(id, values);
    if (isBrowserRuntime()) {
      const saved = await writeInternalCatalog<InternalCategory>('categories', 'PATCH', categoryToCatalogPayload(category));
      return ok(mapInternalCategory(saved));
    }
    await wait();
  ...
  deleteCategory: async (id: string): Promise<ApiResponse<{ success: boolean }>> => {
    if (isBrowserRuntime()) {
      await writeInternalCatalog<{ ok: boolean }>('categories', 'DELETE', { id });
  ...
  togglePublished: async (id: string, published: boolean): Promise<ApiResponse<Category>> => {
    ...
    if (isBrowserRuntime()) {
      const saved = await writeInternalCatalog<InternalCategory>('categories', 'PATCH', categoryToCatalogPayload(updated));
      return ok(mapInternalCategory(saved));
    }
