...
export default function CategoryPage({ storeBase, navItems, slug, products = [], categories = [], settings, routeViews }: { storeBase: string; navItems: NavItem[]; slug: string; products?: ThemeProductCard[]; categories?: ThemeCategoryCard[]; settings?: StorefrontRuntimeSettings; routeViews?: V0ThemeRouteViews }) {
  ...
  if (routeViews?.CategoryPage && settings) {
    const View = routeViews.CategoryPage;
    const safeCategory = category || { slug, title, description, image: '/native-theme-assets/atlantis/hero-slide-2.svg', productCount: shown.length, sortOrder: 0 };
    return <View {...buildV0ThemePageContext({ storeBase, currentPath, navItems, settings })} category={themeCategoryToV0(safeCategory, storeBase)} allProducts={allProducts} products={shown.map((product) => themeProductToV0(product, storeBase))} />;
  }
