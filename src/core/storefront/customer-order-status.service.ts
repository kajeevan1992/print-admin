...
function findForOrder(items: Store[], order: Store) {
...
function deriveCurrentStage(order: Store, ticket: Store | null, plannerJob: Store | null) {
  ...
  if (ticket?.customerProofStatus === 'approved' || ticket?.artworkStatus === 'approved') return 'production';
  if (ticket?.artworkStatus || ticket?.preflightStatus || ticket?.customerProofStatus) return 'proof';
  const rawArtwork = order.rawCheckout?.artwork || order.items?.[0]?.metadataJson?.artworkSnapshot;
  if (rawArtwork?.status || rawArtwork?.upload?.id || order.artworkUploadIds?.length) return 'artwork';
  return 'order';
}
function publicMessage(stage: string, ticket: Store | null, plannerJob: Store | null) {
  const block = text(plannerJob?.blockReason);
...
function progress(stage: string) {
  ...
  return [
    { key: 'order', label: 'Order received' },
    { key: 'artwork', label: 'Artwork check' },
...
function compactPlanner(job: Store | null) {
  if (!job) return null;
  return { id: job.id, stage: job.stage, status: job.status, laneName: job.laneName, dueAt: safeDate(job.dueAt), scheduledStartAt: safeDate(job.scheduledStartAt), scheduledEndAt: safeDate(job.scheduledEndAt), productionBlocked: Boolean(job.productionBlocked), blockReason: job.blockReason || '', liveStatus: job.liveStatus || '', progressPercent: job.progressPercent || 0 };
}
function compactDispatch(job: Store | null) {
  if (!job || !['dispatch', 'completed'].includes(String(job.stage))) return null;
  return { stage: job.stage === 'completed' ? 'handover' : 'ready', carrier: job.carrier || '', service: job.service || '', trackingNumber: job.trackingNumber || '', manifestNumber: job.manifestNumber || '', scanStatus: job.scanStatus || (job.stage === 'completed' ? 'complete' : 'partial') };
}
