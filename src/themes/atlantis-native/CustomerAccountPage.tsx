...
export default async function CustomerAccountPage({ tenantSlug, storeSlug, storeBase, navItems, settings, routeViews, mode, section = 'overview', returnUrl = '', token = '' }: { tenantSlug: string; storeSlug: string; storeBase: string; navItems: NavItem[]; settings: StorefrontRuntimeSettings; routeViews?: V0ThemeRouteViews; mode: Mode; section?: Section; returnUrl?: string; token?: string }) {
  ...
  const isSecurityPage = ['forgot-password', 'reset-password', 'verify-email'].includes(mode);
  ...
  const accountSlot = <CustomerAccountClient mode={resolvedMode} section={section} tenantSlug={tenantSlug} storeSlug={storeSlug} storeBase={storeBase} returnUrl={returnUrl || `${storeBase}/account`} token={token} customer={dashboardCustomer ? { id: dashboardCustomer.id, name: dashboardCustomer.name, email: dashboardCustomer.email, phone: dashboardCustomer.phone, company: dashboardCustomer.company, emailVerified: dashboardCustomer.emailVerified } : null} orders={safeOrders} quotes={safeQuotes} invoices={safeInvoices} addresses={addresses} summary={safeSummary} />;
  if (routeViews?.CustomerAccountPage) { const View = routeViews.CustomerAccountPage; return <View {...buildV0ThemePageContext({ storeBase, currentPath, navItems, settings })} mode={resolvedMode} section={section} authenticated={Boolean(dashboardCustomer)} customer={dashboardCustomer ? { name: dashboardCustomer.name, email: dashboardCustomer.email, emailVerified: dashboardCustomer.emailVerified } : undefined} summary={dashboardCustomer ? { orderCount: summary.orderCount, quoteCount: safeQuotes.length, artworkCount: summary.artworkCount, invoiceCount: safeInvoices.length, addressCount: summary.addressCount } : undefined} slots={{ account: accountSlot }} />; }
