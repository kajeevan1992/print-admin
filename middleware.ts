...
function envOrigins() {
  return [process.env.CORS_ORIGIN, process.env.CORS_ORIGINS, process.env.ALLOWED_ORIGINS, process.env.STOREFRONT_URL, process.env.NEXT_PUBLIC_STOREFRONT_URL]
    .filter(Boolean)
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/internal/') && !isInternalStorefrontApi(pathname) && !hasAdminCookie(request)) {
    return NextResponse.json({ ok: false, error: 'Admin session required.' }, { status: 401 });
  }

  if (isProtectedPage(pathname) && !hasAdminCookie(request)) {
  ...
  if (pathname === '/') { const rewriteUrl = url.clone(); rewriteUrl.pathname = '/theme/atlantis'; return NextResponse.rewrite(rewriteUrl); }
  if (pathname.startsWith('/category/') || pathname.startsWith('/product/') || pathname === '/cart' || pathname === '/checkout') return NextResponse.next();
