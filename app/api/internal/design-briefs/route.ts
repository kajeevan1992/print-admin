...
function proofReviewUrl(request: Request, brief: Store) { const params = new URLSearchParams({ orderId: String(brief.orderNumber || brief.orderId || '') }); if (customerEmail(brief)) params.set('email', customerEmail(brief)); return `${storefrontBase(request)}/proof-action?${params.toString()}`; }
...
function proofReviewEmailOrder(brief: Store) { return { id: text(brief.orderId || brief.orderNumber || brief.id), orderNumber: text(brief.orderNumber || brief.orderId || brief.id), customerName: text(brief.customerName || 'Customer'), customerEmail: customerEmail(brief), currency: 'GBP', totalMinor: 0, status: 'PROOF_READY_FOR_REVIEW', paymentStatus: brief.paymentStatus || 'paid', items: [{ productName: `Proof review${brief.productName ? ` - ${brief.productName}` : ''}`, quantity: 1 }] }; }
...
export async function POST(request: Request) {
  try {
    ...
    const proofEmailResult = designQuoteStatus === 'proof-sent' && sendCustomerEmail ? await queueProofReviewEmail(request, { ...current, designProofUrl: designProofUrl || current.designProofUrl || '' }, designProofUrl || current.designProofUrl || '', staffNote).catch((error) => ({ ok: false, error: error instanceof Error ? error.message : 'Proof review email queue failed.' })) : { skipped: true };
    ...
    const updatedBrief = {
      ...
      designWorkState: designQuoteStatus === 'revision-requested' ? 'proof-revision-requested' : designQuoteStatus === 'revision-in-progress' ? 'revision-in-progress' : current.designWorkState || '',
