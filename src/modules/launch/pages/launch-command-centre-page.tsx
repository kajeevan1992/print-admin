...
export function LaunchCommandCentrePage() {
  ...
  const abuseBlocked = countFrom(abuseSummary, ['blocked', 'fail']);
  const abuseWarn = countFrom(abuseSummary, ['warn', 'warnings', 'review']);
  ...
  return <div>
    ...
    <div className="mb-4 grid gap-4 md:grid-cols-6">
      ...
      <Metric label="Abuse risk" value={abuseBlocked ? `${abuseBlocked} blocked` : abuseWarn ? `${abuseWarn} review` : 'Clear'} tone={abuseBlocked ? 'bad' : abuseWarn ? 'warn' : 'good'} />
