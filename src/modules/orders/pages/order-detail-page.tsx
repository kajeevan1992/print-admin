...
type ArtworkUpload = {
  ...
  approvalHistory?: Array<{ id: string; action: ArtworkReviewStatus | string; actor: string; note?: string; createdAt: string }>;
  createdAt: string;
};
...
function statusTone(status?: string) {
  if (status === 'approved' || status === 'ready-to-print' || status === 'printing' || status === 'finishing' || status === 'packing' || status === 'dispatched') return 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200';
  if (status === 'rejected' || status === 'replacement-requested' || status === 'blocked') return 'border-red-500/30 bg-red-500/10 text-red-200';
  ...
  async function updateArtworkStatus(upload: ArtworkUpload, action: ArtworkReviewStatus) {
    ...
    try {
      const res = await fetch(`/api/internal/storefront/artwork/uploads/${upload.id}/status`, {
        ...
        body: JSON.stringify({ action, note: reviewNote, actor: 'admin', orderId: order.id, orderNumber: order.orderNumber, customerName: order.customerName, customerEmail: order.customerEmail, productName: order.items?.[0]?.productName || '' }),
      });
      const payload = await res.json().catch(() => ({}));
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<><Button>Download Proof</Button>{primaryProductionJob ? <a href={`/production/jobs/${primaryProductionJob.id}`}><PrimaryButton><ExternalLink size={14} /> Open Production Job</PrimaryButton></a> : <PrimaryButton disabled>Open Production Job</PrimaryButton>}</>}
      />
      ...
      {activeTab === 'Artwork' ? (
        <div className="grid gap-4 xl:grid-cols-[1.25fr_1fr]">
          <Card>
            <div className="mb-4 flex items-center justify-between gap-3"><h3 className="text-sm font-semibold">Order Artwork</h3><Button onClick={() => { void loadArtwork(order); void loadProductionJobs(order); }}><RefreshCw size={14} /> Refresh</Button></div>
            ...
            <div className="space-y-3">{artworkUploads.map((upload) => <ArtworkUploadCard key={upload.id} upload={upload} productionJob={productionJobByArtworkId.get(upload.id) || productionJobs.find((job) => job.id === upload.productionJobId)} onStatus={(action) => void updateArtworkStatus(upload, action)} busy={reviewingId === upload.id} />)}</div>
          </Card>
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <MetricMini label="Production Jobs" value={String(productionJobs.length)} />
            </div>
            ...
            <p className="mt-4 text-sm leading-6 text-textMuted">Approving artwork now creates or links a production job ticket. Use the production job link to manage printing, finishing, packing and dispatch.</p>
          </Card>
        </div>
      ) : null}
...
function ProductionJobsPanel({ loading, jobs, orderStage, onRefresh }: { loading: boolean; jobs: ProductionJobTicket[]; orderStage: string; onRefresh: () => void }) {
...
function ArtworkUploadCard({ upload, productionJob, onStatus, busy }: { upload: ArtworkUpload; productionJob?: ProductionJobTicket; onStatus: (status: ArtworkReviewStatus) => void; busy: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-panelMuted p-4">
      ...
      <div className="mt-3 rounded-lg border border-border bg-panel px-3 py-3 text-sm text-textMuted">
        ...
        {productionJob ? <p className="mt-2 text-white">Linked production job: {productionJob.id}</p> : null}
      </div>
      {upload.approvalHistory?.length ? <div className="mt-3 space-y-1 text-xs text-textMuted">{upload.approvalHistory.slice(-3).map((event) => <p key={event.id}>• {new Date(event.createdAt).toLocaleString()} — {event.actor}: {String(event.action).replace(/-/g, ' ')}{event.note ? ` — ${event.note}` : ''}</p>)}</div> : null}
    </div>
  );
}
