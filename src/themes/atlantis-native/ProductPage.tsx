...
function imageList(product: Record<string, any>, fallback?: ThemeProductCard | null) {
  const media = product.metadataJson?.media || product.media || {};
  const direct = [product.image, product.imageUrl, product.thumbnail, product.heroImage, product.metadataJson?.image, media.heroImageUrl, fallback?.image].filter(Boolean).map(String);
...
function contentText(product: Record<string, any>, fallback?: ThemeProductCard | null) {
  const content = product.metadataJson?.content || product.content || {};
  return clean(content.shortDescription || product.description || product.subtitle || fallback?.text || '');
}
function isQuoteProduct(product: Record<string, any>, fallback?: ThemeProductCard | null) {
  const mode = clean(product.buyingMode || product.orderMode || product.metadataJson?.buyingMode || product.metadataJson?.orderMode || fallback?.buyingMode).toLowerCase();
  const type = clean(product.productType || product.type || product.metadataJson?.productType || fallback?.productType).toUpperCase();
...
async function loadBackendProductContract(tenantSlug: string, slug: string, fallback?: ThemeProductCard | null) {
  try {
    ...
    const images = imageList(product, fallback || null);
    const title = clean(product.title || product.name || fallback?.title || slug);
    const description = contentText(product, fallback || null);
    return {
      product,
      title,
      description,
      ...
      category: clean(product.categorySlug || product.metadataJson?.categorySlug || fallback?.category || 'all-products'),
      buyingMode: isQuoteProduct(product, fallback || null) ? 'quote' : 'cart',
      priceText: initialPrice(product, resolvedConfig)?.formattedPrice || fallback?.price || '',
      resolvedConfig,
...
export default async function ProductPage({ tenantSlug, storeSlug, storeBase, navItems, slug, category, products = [], searchParams = {} }: { tenantSlug: string; storeSlug: string; storeBase: string; navItems: NavItem[]; slug: string; category: string; products?: ThemeProductCard[]; searchParams?: Record<string, string> }) {
  const fallbackProduct = products.find((item) => item.slug === slug && item.category === category) || null;
  const backend = await loadBackendProductContract(tenantSlug, slug, fallbackProduct);
  const product = backend || (fallbackProduct ? { product: fallbackProduct as any, title: fallbackProduct.title, description: fallbackProduct.text, images: fallbackProduct.image ? [fallbackProduct.image] : [], image: fallbackProduct.image, category: fallbackProduct.category, buyingMode: fallbackProduct.buyingMode || 'cart', priceText: fallbackProduct.price, resolvedConfig: { customerGroups: fallbackProduct.optionGroups || [], quantityRows: [], deliveryRows: [], selections: {}, selectedQuantity: null, selectedDelivery: null }, initialPrice: null } : null);
