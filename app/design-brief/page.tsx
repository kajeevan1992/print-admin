...
import { CheckCircle2, ClipboardList, CreditCard, Loader2, Palette, Search } from 'lucide-react';

type OrderState = { id?: string; orderNumber?: string; customerName?: string; customerEmail?: string; paymentStatus?: string };
function text(value: unknown) { return String(value || '').trim(); }
function moneyMinor(value: unknown) { const amount = Number(value || 0); return amount > 0 ? new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(amount / 100) : 'Not set'; }
function paid(value: unknown) { return ['paid', 'captured', 'authorized', 'manual-paid'].includes(String(value || '').toLowerCase()); }
...
export default function DesignBriefPage() {
  ...
  const latestBrief = useMemo(() => existingBriefs[0] || null, [existingBriefs]);
  const quoteUrl = text(latestBrief?.designQuotePaymentUrl || latestBrief?.ticket?.designQuotePaymentUrl);
  const quoteStatus = text(latestBrief?.designQuotePaymentStatus || latestBrief?.ticket?.designQuotePaymentStatus || latestBrief?.designQuoteStatus || '');
  const quotePaid = paid(quoteStatus) || text(latestBrief?.designQuoteStatus) === 'approved-to-design';
  const quoteAmount = latestBrief?.quoteAmountMinor || latestBrief?.ticket?.designQuoteAmountMinor || latestBrief?.designQuoteAmountMinor || 0;
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl space-y-6"><section className="rounded-[32px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.18),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(2,6,23,0.96))] p-7 shadow-[0_24px_80px_rgba(0,0,0,0.28)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.25em] text-sky-200">Design help</p><h1 className="mt-2 text-4xl font-black tracking-[-0.06em]">Complete your design brief</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Tell our design team what you need. Your print order can stay paid/held while staff review the brief and confirm any extra design charge before design starts.</p></div><div className="grid h-14 w-14 place-items-center rounded-3xl border border-sky-400/25 bg-sky-400/10 text-sky-100"><Palette size={24}/></div></div></section>
    ...
    {order ? <section className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]"><aside className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2 text-sky-100"><ClipboardList size={18}/><h2 className="font-black">Order</h2></div><div className="mt-4 space-y-3 text-sm"><p><span className="text-slate-500">Order:</span> <span className="font-black">{order.orderNumber || order.id}</span></p><p><span className="text-slate-500">Customer:</span> {order.customerName || 'Customer'}</p><p><span className="text-slate-500">Print payment:</span> {order.paymentStatus || 'checking'}</p></div>{existingBriefs.length ? <div className="mt-6 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-4 text-sm text-emerald-100"><p className="font-black">Design brief already submitted</p><p className="mt-1 text-emerald-100/80">You can submit another update if anything has changed.</p></div> : null}{latestBrief ? <div className={`mt-4 rounded-2xl border p-4 text-sm ${quotePaid ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-100' : quoteUrl ? 'border-amber-400/20 bg-amber-400/10 text-amber-100' : 'border-white/10 bg-white/[0.04] text-slate-300'}`}><div className="flex items-center gap-2"><CreditCard size={17}/><p className="font-black">Design quote</p></div><p className="mt-2">Status: <span className="font-black">{quotePaid ? 'paid' : quoteStatus || 'waiting staff review'}</span></p><p className="mt-1">Amount: <span className="font-black">{moneyMinor(quoteAmount)}</span></p>{quoteUrl && !quotePaid ? <a href={quoteUrl} className="mt-4 inline-flex w-full justify-center rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950">Pay design quote</a> : null}{quotePaid ? <p className="mt-3 text-emerald-100/80">Your design quote is paid. Design can start; print production remains blocked until final proof approval.</p> : quoteUrl ? <p className="mt-3 text-amber-100/80">Please pay this extra design charge before design work starts.</p> : <p className="mt-3 text-slate-400">Staff will review and confirm whether an extra design charge is needed.</p>}</div> : null}<a href={`/track-order?orderId=${encodeURIComponent(order.orderNumber || order.id || orderId)}${email ? `&email=${encodeURIComponent(email)}` : ''}`} className="mt-5 inline-flex w-full justify-center rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-bold text-white">Back to Track Order</a></aside>
