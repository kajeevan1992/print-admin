...
// Extension registry for new/admin tools. It is merged in this config file only.
// Components must never merge or hardcode navigation themselves.
...
function isRoleVisible(roles: AdminRole[] | undefined, role?: string | null) {
  const normalizedRole = normalizeAdminRole(role);
  return !roles?.length || roles.includes(normalizedRole);
}

function isCommonVisibilityAllowed(
  item: { hidden?: boolean; roles?: AdminRole[]; ownerOnly?: boolean; tenantOnly?: boolean; featureFlagKey?: string; surface?: AdminNavigationSurface },
  role?: string | null,
  ...
  const normalizedRole = normalizeAdminRole(role);
  if (!isRoleVisible(item.roles, normalizedRole)) return false;
  if (item.ownerOnly && normalizedRole !== 'owner' && normalizedRole !== 'super_admin') return false;
  if (item.tenantOnly && (normalizedRole === 'owner' || normalizedRole === 'super_admin')) return false;
  if (item.featureFlagKey && !enabledFeatureFlags.includes(item.featureFlagKey)) return false;
  if (surface !== 'any') {
...
export function isRegistryItemVisible(
  ...
  return isCommonVisibilityAllowed(item, role, enabledFeatureFlags, surface);
}
...
function cloneSidebarItem(item: AdminSidebarNavigationItem): AdminSidebarNavigationItem {
  return {
    ...item,
...
function sortSidebarItems<T extends { label: string; order?: number }>(items: T[]) {
  return [...items].sort((a, b) => (a.order ?? 999) - (b.order ?? 999) || a.label.localeCompare(b.label));
}

function hrefExists(items: AdminSidebarNavigationItem[], href: string) {
  return items.some((item) => item.href === href || item.children?.some((child) => child.href === href));
}

function mergeRegistryItems(items: AdminSidebarNavigationItem[], registryItems: AdminNavigationRegistryItem[]) {
  const next = items.map(cloneSidebarItem);

  for (const registryItem of registryItems) {
    if (hrefExists(next, registryItem.href)) continue;

    if (registryItem.parentLabel) {
      const parent = next.find((item) => item.label === registryItem.parentLabel);
      if (parent) {
        parent.children = sortSidebarItems([
          ...(parent.children ?? []),
          {
            label: registryItem.label,
            href: registryItem.href,
            iconKey: registryItem.iconKey,
            order: registryItem.order,
            roles: registryItem.roles,
            ownerOnly: registryItem.ownerOnly,
            tenantOnly: registryItem.tenantOnly,
            hidden: registryItem.hidden,
            featureFlagKey: registryItem.featureFlagKey,
            surface: registryItem.surface
          }
        ]);
        continue;
      }
    ...
    const navItem: AdminSidebarNavigationItem = {
      label: registryItem.label,
      href: registryItem.href,
      iconKey: registryItem.iconKey ?? 'FileText',
      order: registryItem.order,
      roles: registryItem.roles,
      ownerOnly: registryItem.ownerOnly,
      tenantOnly: registryItem.tenantOnly,
    ...
    if (insertAfterIndex >= 0) next.splice(insertAfterIndex + 1, 0, navItem);
    else next.push(navItem);
  }

  return sortSidebarItems(next).map((item) => ({
    ...item,
    children: item.children ? sortSidebarItems(item.children) : undefined
  }));
}
...
export function getAdminSidebarNavigation(
  role?: string | null,
  enabledFeatureFlags: string[] = [],
  surface: AdminNavigationSurface | 'any' = 'sidebar'
): AdminSidebarNavigationItem[] {
  const normalizedRole = normalizeAdminRole(role);
  const baseItems = normalizedRole === 'super_admin' ? SUPER_ADMIN_SIDEBAR_NAVIGATION : ADMIN_SIDEBAR_NAVIGATION;
  const visibleBase = baseItems
    .filter((item) => isCommonVisibilityAllowed(item, normalizedRole, enabledFeatureFlags, surface))
    .map((item) => ({
      ...cloneSidebarItem(item),
      children: item.children?.filter((child) => isCommonVisibilityAllowed(child, normalizedRole, enabledFeatureFlags, surface))
    }));

  return mergeRegistryItems(visibleBase, getVisibleAdminNavigationRegistry(normalizedRole, enabledFeatureFlags, surface));
}
...
export function validateAdminNavigationRegistry(items: AdminNavigationRegistryItem[] = ADMIN_NAVIGATION_REGISTRY) {
  ...
  for (const item of items) {
    ...
    if (item.parentLabel && item.insertAfterLabel) warnings.push(`${item.label} has both parentLabel and insertAfterLabel; parentLabel wins.`);
...
export function validateAdminSidebarNavigation(items: AdminSidebarNavigationItem[] = getAdminSidebarNavigation('admin')) {
  const seenHref = new Set<string>();
  const errors: string[] = [];
  const warnings: string[] = [];

  function visit(item: AdminSidebarNavigationItem | NonNullable<AdminSidebarNavigationItem['children']>[number], parent?: string) {
    if (!item.label?.trim()) errors.push(`Navigation item${parent ? ` under ${parent}` : ''} is missing label.`);
    if ('href' in item && item.href) {
      const hrefKey = item.href.toLowerCase();
      if (!item.href.startsWith('/')) errors.push(`${item.label} must use an absolute href.`);
      if (seenHref.has(hrefKey)) errors.push(`Duplicate sidebar href: ${item.href}`);
      seenHref.add(hrefKey);
    }
    if ('children' in item) item.children?.forEach((child) => visit(child, item.label));
  }
