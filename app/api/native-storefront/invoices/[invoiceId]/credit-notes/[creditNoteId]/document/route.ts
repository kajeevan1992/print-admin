...
export async function GET(request: NextRequest, { params }: { params: { invoiceId: string; creditNoteId: string } }) {
  try {
    ...
    if (!tenantSlug || !storeSlug) return NextResponse.json({ ok: false, error: 'Missing storefront credit-note scope.' }, { status: 400 });
    ...
    if (!invoice || !owned || !creditNote) return NextResponse.json({ ok: false, error: 'Credit note was not found in this account.' }, { status: 404 });
