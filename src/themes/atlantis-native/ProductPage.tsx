...
function queryFromSelected(options: Record<string, string>) {
...
function productAction(product: ThemeProductCard, storeBase: string, category: string, slug: string, searchParams: Record<string, string>) {
  ...
  const optionQueryPart = queryFromSelected(options);
  return {
    label: isQuote ? 'Request quote' : 'Add to cart',
    href: isQuote ? `${storeBase}/quote/${category}/${slug}?${new URLSearchParams(options).toString()}` : `${storeBase}/cart?product=${encodeURIComponent(slug)}&category=${encodeURIComponent(category)}${optionQueryPart}`,
