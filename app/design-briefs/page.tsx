...
function statusTone(value: string) {
  ...
  if (['quote-sent'].includes(value)) return 'border-sky-300 bg-sky-50 text-sky-800';
  ...
  async function save() {
    ...
    try {
      const response = await fetch('/api/internal/design-briefs', {
        ...
        body: JSON.stringify({ id: brief.id, designQuoteStatus: status, quoteAmountMinor: Math.round(Number(amount || 0) * 100), staffNote: note, actor: 'admin-design-review' }),
      });
