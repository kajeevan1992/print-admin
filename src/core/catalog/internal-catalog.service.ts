...
function catalogConnectionFromDatabaseUrl(): PostgresConnectionInput | null {
  ...
  try {
    ...
    const sslMode = sslModeParam === 'disable' || sslModeParam === 'require' ? sslModeParam : 'prefer';
...
async function getCatalogConnection(ctx: TenantContext) {
  ...
  const input = connection?.encryptedPassword ? toConnectionInput(connection) : catalogConnectionFromDatabaseUrl();
...
async function requireCatalogConnection(ctx: TenantContext) {
  ...
  if (!connectionInput) {
    throw new Error(
      `No writable tenant database configured for tenant ${ctx.tenantId}. Add a tenant database in Database Manager or set TENANT_DATABASE_URL/DATABASE_URL.`
    );
  }
...
export async function deleteInternalCatalogRecord(ctx: TenantContext, resource: CatalogResource, id: string) {
  ...
  return withPgClient(connectionInput, async (client) => {
    ...
    if (resource === 'categories') {
      const current = await readCategoryByIdOrSlug(client, ctx.tenantId, id);
      if (current?.id) {
        await client.query('UPDATE "Product" SET "categoryId" = NULL, "updatedAt" = CURRENT_TIMESTAMP WHERE "tenantId" = $1 AND "categoryId" = $2', [ctx.tenantId, current.id]);
      }
