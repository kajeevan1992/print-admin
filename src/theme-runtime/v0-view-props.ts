...
type V0DropdownNavigationItem = V0ThemeNavigationItem & {
  description: string;
  image: string;
  groups: Array<{ title: string; links: Array<V0ThemeNavigationItem> }>;
};
...
export function buildV0ThemeNavigation(basePath: string, navItems: NavItem[], currentPath: string): V0ThemeNavigationItem[] {
  const navigation: V0DropdownNavigationItem[] = navItems.map((item) => ({
    label: item.label,
  ...
  if (!navigation.some((item) => item.href === href(basePath, '/search'))) navigation.push({ label: 'Search', href: href(basePath, '/search'), active: currentPath === '/search', description: '', image: '', groups: [] });
  return navigation;
}
