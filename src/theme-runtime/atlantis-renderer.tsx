...
export async function renderAtlantisStorefront(context: StorefrontRuntimeContext) {
  ...
  if (routeSegments.length >= 2) return <ProductPage tenantSlug={tenantSlug} storeSlug={storeSlug} storeBase={storeBase} navItems={navItems} category={routeSegments[0]} slug={routeSegments[routeSegments.length - 1]} products={products} searchParams={searchParams || {}} />;
  return <CategoryPage storeBase={storeBase} navItems={navItems} slug={routeSegments[0]} products={products} categories={categories} />;
}
