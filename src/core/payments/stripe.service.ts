...
async function syncTicketPayment(request: Request, order: any, status: string, note: string) {
  ...
  const next = items.map((ticket) => {
    ...
    return {
      ...
      paidAt: ['paid', 'captured', 'authorized'].includes(status) ? now : ticket.paidAt || '',
...
export async function applyStripePaymentIntentToOrder(request: Request, intent: any, eventType = 'manual') {
  ...
  const updated = await updateOrder(request, order.id, { status: nextStatus, paymentStatus: nextPaymentStatus, paymentProvider: 'stripe', stripePaymentIntentId: intent.id || order.stripePaymentIntentId || '', paidAt: paid ? new Date().toISOString() : order.paidAt, paymentFailureReason: failed ? eventType : '', internalNotes: noteList(order, note) });
  await syncTicketPayment(request, updated, nextPaymentStatus, note).catch(() => null);
  return { ok: true, order: updated, paid, authorized, failed, eventType };
}
...
export async function applyStripeRefundToOrder(request: Request, refund: any, eventType = 'manual') {
  ...
  const updated = await updateOrder(request, order.id, { paymentStatus: nextPaymentStatus, paymentProvider: 'stripe', stripeRefundId: refund.id || order.stripeRefundId || '', stripeRefundStatus: refund.status || order.stripeRefundStatus || '', refundAmountMinor: refund.amount || order.refundAmountMinor || '', refundedAt: succeeded ? new Date().toISOString() : order.refundedAt, refundNote: refund.reason || order.refundNote || '', internalNotes: noteList(order, note) });
  await syncTicketPayment(request, updated, nextPaymentStatus, note).catch(() => null);
  return { ok: true, order: updated, refunded: succeeded, eventType };
}
