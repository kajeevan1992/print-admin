...
function slug(value: string) { return clean(value).toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/(^-|-$)/g, ''); }
...
function objectOptionsToRows(value: unknown): NativeSelectedOptionRow[] { if (!value || typeof value !== 'object' || Array.isArray(value)) return []; return Object.entries(value as Record<string, unknown>).map(([key, optionValue]) => ({ key, label: key, value: String(optionValue || ''), slug: String(optionValue || '') })).filter((row) => row.value); }

export async function POST(request: NextRequest) {
  try {
    const body = await request.json().catch(() => ({}));
    const tenantSlug = slug(body.tenantSlug || body.tenantId || '');
    ...
    if (!tenantSlug || !productSlug) {
      return NextResponse.json({ ok: false, error: 'Missing tenantSlug or productSlug.' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ok: true,
      source: 'native-storefront-live-pricing',
      data: {
        product: {
          id: price.product.id,
    ...
    return NextResponse.json({
      ok: false,
      source: 'native-storefront-live-pricing',
      error: error instanceof Error ? error.message : 'Price could not be calculated.',
    }, { status: 500 });
  }
}
