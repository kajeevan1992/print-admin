...
function isReadyForDecision(status: StatusPayload | null) {
  const proof = status?.artwork || {};
  if (proofDecisionClosed(status)) return false;
  const proofStatus = clean(proof.customerProofStatus);
  const artworkStatus = clean(proof.artworkStatus);
...
export default function ProofActionPage() {
  ...
  async function submit(decision: 'approve' | 'revision') {
    if (!isReadyForDecision(status)) {
      setError('This proof is no longer open for a customer decision. Please wait for the latest proof or contact the store.');
      return;
    }
    setBusy(decision);
    setMessage('');
  ...
  const readyForDecision = isReadyForDecision(status);
