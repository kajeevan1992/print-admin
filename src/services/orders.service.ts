...
function internalPaymentToAdmin(value: unknown): PaymentStatus { const raw = String(value || '').toLowerCase(); if (raw.includes('refund-pending')) return 'refund-pending'; if (raw.includes('refund')) return 'refunded'; if (raw.includes('failed')) return 'failed'; if (raw.includes('paid')) return 'paid'; if (raw.includes('author')) return 'authorized'; return 'unpaid'; }
...
function adminPaymentToInternal(status: PaymentStatus) { const map: Record<PaymentStatus, string> = { unpaid: 'AWAITING_PAYMENT', authorized: 'AWAITING_PAYMENT', paid: 'APPROVED', failed: 'AWAITING_PAYMENT', 'refund-pending': 'APPROVED', refunded: 'CANCELLED' }; return map[status] || 'AWAITING_PAYMENT'; }
...
async function paymentAction(id: string, body: Record<string, any>): Promise<Order | null> { if (typeof window === 'undefined') return null; const res = await fetch(`/api/internal/orders/${encodeURIComponent(id)}/payment`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const payload = await res.json().catch(() => null); if (!res.ok || !payload?.ok) throw new Error(payload?.error || 'Payment action failed'); const raw = payload?.order || payload?.data?.order; return raw ? normaliseInternalOrder(raw) : null; }

type AdminPaymentAction = 'mark-paid' | 'mark-failed' | 'mark-refunded' | 'refund-note' | 'stripe-refund';
...
export const ordersService = {
  ...
  adminPaymentAction: async (id: string, body: { action: AdminPaymentAction; note?: string; reference?: string; amountMinor?: number; actor?: string; paymentProvider?: string; reason?: 'duplicate' | 'fraudulent' | 'requested_by_customer' }): Promise<ApiResponse<Order>> => { const patched = await paymentAction(id, body); if (patched) return ok(patched); const current = readStore().find((order) => order.id === id) ?? ordersStore[0]; const paymentStatus = body.action === 'mark-paid' ? 'paid' : body.action === 'mark-failed' ? 'failed' : body.action === 'mark-refunded' ? 'refunded' : body.action === 'stripe-refund' ? 'refund-pending' : current.paymentStatus; return ordersService.saveOrder({ ...current, paymentStatus, refundNote: body.note || current.refundNote, updatedAt: new Date().toISOString() }); },
