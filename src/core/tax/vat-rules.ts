...
type ProductTaxSettingsLike = {
  taxClass?: 'auto' | 'zero' | 'standard' | 'exempt' | 'custom' | string;
...
function combinedText(item: Record<string, any>) {
  ...
  return [
    ...
    resolver.product?.name, resolver.product?.title, resolver.product?.categoryName, resolver.product?.categorySlug,
  ].map(text).filter(Boolean).join(' ');
}
function taxSettingsFrom(item: Record<string, any>): ProductTaxSettingsLike | null {
  const meta = item.metadataJson || item.metadata || {};
  const config = item.config || item.selections || item.options || {};
  const resolver = item.resolverSnapshot || {};
  return item.taxSettings || meta.taxSettings || config.taxSettings || resolver.product?.taxSettings || resolver.taxSettings || null;
}
function readExplicitRate(item: Record<string, any>) {
  const meta = item.metadataJson || item.metadata || {};
  const config = item.config || item.selections || item.options || {};
  const resolver = item.resolverSnapshot || {};
  const taxSettings = taxSettingsFrom(item);
  const candidates = [
    ...
    taxSettings?.taxClass === 'custom' ? taxSettings?.vatRate : undefined,
  ...
  for (const candidate of candidates) { const rate = numberOrNull(candidate); if (rate !== null) return rate; }
  return null;
}
function readTaxClass(item: Record<string, any>) {
  ...
  const resolver = item.resolverSnapshot || {};
  const taxSettings = taxSettingsFrom(item);
  return text(item.vatClass || item.taxClass || meta.vatClass || meta.taxClass || config.vatClass || config.taxClass || taxSettings?.taxClass || resolver.product?.vatClass || resolver.product?.taxClass || resolver.pricing?.taxClass || '');
}
...
function profileFromTaxSettings(settings: ProductTaxSettingsLike | null, sourceText: string): VatProfile | null {
  ...
  const forceServiceVat = settings.forceVatOnDesignServices !== false;
  if (forceServiceVat && hasAny(sourceText, STANDARD_SERVICE_TERMS)) return { vatRate: 20, vatClass: 'standard', vatReason: 'product-tax-settings-design-service-override' };
  if (taxClass === 'standard') return { vatRate: 20, vatClass: 'standard', vatReason: 'product-tax-settings-standard' };
  ...
  if (taxClass === 'custom') {
    const rate = numberOrNull(settings.vatRate) ?? 0;
    return { vatRate: rate, vatClass: rate === 0 ? 'zero' : rate === 20 ? 'standard' : 'custom', vatReason: 'product-tax-settings-custom-rate' };
  }
  ...
  if (['business cards', 'signage banners', 'stickers labels', 'design service'].includes(preset)) return { vatRate: 20, vatClass: 'standard', vatReason: 'product-tax-settings-standard-rated-preset' };
  return null;
}
...
export function calculateDeliveryVat(delivery: Record<string, any> | undefined, shippingGrossMinor: number) {
  ...
  return { vatRate, vatClass: vatRate === 0 ? 'zero' : vatRate === 20 ? 'standard' : 'custom', vatReason: explicit !== null ? 'explicit-delivery-vat-rate' : 'delivery-tax-class', grossMinor, netMinor, vatMinor: Math.max(0, grossMinor - netMinor) };
}
