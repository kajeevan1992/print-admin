...
function internalPaymentToAdmin(value: unknown): PaymentStatus { const raw = String(value || '').toLowerCase(); if (raw.includes('refund-pending')) return 'refund-pending'; if (raw.includes('refund')) return 'refunded'; if (raw.includes('failed')) return 'failed'; if (raw.includes('paid')) return 'paid'; if (raw.includes('pending')) return 'authorized'; if (raw.includes('author')) return 'authorized'; return 'unpaid'; }
...
export const ordersService = {
  ...
  adminPaymentAction: async (id: string, body: { action: AdminPaymentAction; note?: string; reference?: string; amountMinor?: number; actor?: string; paymentProvider?: string; reason?: 'duplicate' | 'fraudulent' | 'requested_by_customer'; storefrontUrl?: string; successUrl?: string; cancelUrl?: string; customerEmail?: string }): Promise<ApiResponse<Order> & { paymentUrl?: string; session?: any }> => { const result = await paymentAction(id, body); if (result.order) return { ...ok(result.order), paymentUrl: result.paymentUrl, session: result.session }; const current = readStore().find((order) => order.id === id) ?? ordersStore[0]; const paymentStatus = body.action === 'mark-paid' ? 'paid' : body.action === 'mark-failed' ? 'failed' : body.action === 'mark-refunded' ? 'refunded' : body.action === 'stripe-refund' ? 'refund-pending' : body.action === 'create-payment-link' ? 'authorized' : current.paymentStatus; return { ...await ordersService.saveOrder({ ...current, paymentStatus, refundNote: body.note || current.refundNote, updatedAt: new Date().toISOString() }), paymentUrl: result.paymentUrl, session: result.session }; },
