...
function statusTone(status?: string) { if (status === 'paid' || status === 'approved' || status === 'ready-to-print' || status === 'printing' || status === 'finishing' || status === 'packing' || status === 'dispatched') return 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200'; if (status === 'failed' || status === 'refunded' || status === 'rejected' || status === 'replacement-requested' || status === 'blocked') return 'border-red-500/30 bg-red-500/10 text-red-200'; return 'border-amber-500/30 bg-amber-500/10 text-amber-200'; }
...
export function OrderDetailPage({ id }: { id: string }) {
  ...
  async function runPaymentAction(action: 'mark-paid' | 'mark-failed' | 'mark-refunded' | 'refund-note') {
    if (!order) return;
    setSavingPayment(true); setError(null);
    try {
      const amountMinor = refundAmount ? Math.round(Number(refundAmount) * 100) : undefined;
      const response = await ordersService.adminPaymentAction(order.id, { action, note: paymentNote, reference: paymentReference, amountMinor, actor: 'admin', paymentProvider: order.paymentProvider || 'manual' });
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-[2fr_1fr]">
        <Card>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <Select options={['unpaid', 'authorized', 'paid', 'failed', 'refunded']} value={order.paymentStatus} onChange={(e) => void updatePayment(e.target.value as PaymentStatus)} />
      ...
      <PaymentAdminPanel order={order} paymentNote={paymentNote} setPaymentNote={setPaymentNote} paymentReference={paymentReference} setPaymentReference={setPaymentReference} refundAmount={refundAmount} setRefundAmount={setRefundAmount} saving={savingPayment} onAction={(action) => void runPaymentAction(action)} />
      ...
      {activeTab === 'Payment' ? <PaymentAdminPanel order={order} paymentNote={paymentNote} setPaymentNote={setPaymentNote} paymentReference={paymentReference} setPaymentReference={setPaymentReference} refundAmount={refundAmount} setRefundAmount={setRefundAmount} saving={savingPayment} onAction={(action) => void runPaymentAction(action)} expanded /> : null}
...
function PaymentAdminPanel({ order, paymentNote, setPaymentNote, paymentReference, setPaymentReference, refundAmount, setRefundAmount, saving, onAction, expanded = false }: { order: Order; paymentNote: string; setPaymentNote: (v: string) => void; paymentReference: string; setPaymentReference: (v: string) => void; refundAmount: string; setRefundAmount: (v: string) => void; saving: boolean; onAction: (action: 'mark-paid' | 'mark-failed' | 'mark-refunded' | 'refund-note') => void; expanded?: boolean }) {
  return <Card className="mb-4"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold">Payment Status</h3><p className="mt-1 text-xs text-textMuted">Manual payment actions update the same order payment metadata used by Stripe Checkout.</p></div><span className={`rounded-full border px-3 py-1 text-xs capitalize ${statusTone(order.paymentStatus)}`}>{order.paymentStatus}</span></div><div className="grid gap-3 lg:grid-cols-4"><MetricMini label="Provider" value={order.paymentProvider || 'manual/unknown'} /><MetricMini label="Reference" value={order.paymentReference || order.stripeCheckoutSessionId || 'not set'} /><MetricMini label="Paid at" value={order.paidAt ? new Date(order.paidAt).toLocaleString() : 'not paid'} /><MetricMini label="Refund" value={order.refundedAt ? 'refunded' : order.refundNote ? 'note added' : 'none'} /></div>{expanded ? <div className="mt-4 grid gap-3 rounded-xl border border-border bg-panelMuted p-4 text-sm text-textMuted"><p>Stripe session: {order.stripeCheckoutSessionId || 'not set'}</p><p>Payment intent: {order.stripePaymentIntentId || 'not set'}</p><p>Failure reason: {order.paymentFailureReason || 'none'}</p><p>Refund note: {order.refundNote || 'none'}</p><p>Refund amount: {moneyMinor(order.refundAmountMinor) ? `£${(moneyMinor(order.refundAmountMinor) / 100).toFixed(2)}` : 'not set'}</p></div> : null}<div className="mt-4 grid gap-3 md:grid-cols-3"><Input placeholder="Payment/refund reference" value={paymentReference} onChange={(e) => setPaymentReference(e.target.value)} /><Input placeholder="Refund amount e.g. 12.50" value={refundAmount} onChange={(e) => setRefundAmount(e.target.value)} /><Input placeholder="Admin note" value={paymentNote} onChange={(e) => setPaymentNote(e.target.value)} /></div><div className="mt-4 flex flex-wrap gap-2"><PrimaryButton onClick={() => onAction('mark-paid')} disabled={saving}>Mark Paid</PrimaryButton><Button onClick={() => onAction('mark-failed')} disabled={saving}>Mark Failed</Button><Button onClick={() => onAction('refund-note')} disabled={saving}>Add Refund Note</Button><Button onClick={() => onAction('mark-refunded')} disabled={saving}>Mark Refunded</Button></div></Card>;
}
