...
export async function GET() {
  ...
  return json({ ok: true, source: 'internal-invoice-settings', data: { settings }, settings });
}

async function handleWrite(request: NextRequest) {
  try {
    const body = await request.json().catch(() => ({}));
    const settings = await saveInvoiceSettings(body.settings || body);
    return json({ ok: true, source: 'internal-invoice-settings', data: { settings }, settings });
  } catch (error) {
    return json({ ok: false, source: 'internal-invoice-settings', error: error instanceof Error ? error.message : 'Failed to save invoice settings.' }, { status: 500 });
  }
}
