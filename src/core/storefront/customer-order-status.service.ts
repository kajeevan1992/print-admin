...
function compactTicket(ticket: Store | null, order: Store) {
  ...
  const currentProofVersion = proofVersion(ticket);
  const token = text(ticket.proofToken || ticket.currentProofToken || '');
  const proofParams = { ...(token ? { proofToken: token } : {}), ...(currentProofVersion ? { proofVersion: String(currentProofVersion) } : {}) };
  const needsCustomerDecision = ticket.customerProofStatus === 'pending-customer-approval' && (['preflight-pass', 'preflight-warning', 'design-proof-ready'].includes(String(ticket.artworkStatus)) || ['pass', 'warning'].includes(cleanStatus(ticket.preflightStatus)));
  ...
  const paymentReleased = ticketPaymentReleased(ticket, order);
  const needsDesignBrief = designHelpRequested(ticket, order) && !designBriefSubmitted(ticket);
  const quotePaid = designQuotePaymentPaid(ticket);
