...
export default function HOLOV2HomePage(props: V0ThemeHomeProps) {
  ...
  return <div style={{ backgroundColor: props.brand.background, color: props.brand.text }} data-v0-theme-package="holo-v2">
    ...
    <main>
      <section className="border-b" style={{ borderColor: props.brand.border }}><div className="mx-auto w-full max-w-[1320px] px-5 py-16 md:px-8 lg:py-24"><div className="text-[11px] font-black uppercase tracking-[0.18em]" style={{ color: props.brand.primary }}>HOLO V2</div><h1 className="mt-4 max-w-[820px] text-[52px] font-black leading-[0.92] tracking-[-0.07em] sm:text-[72px]">{title}</h1>{body ? <p className="mt-6 max-w-[660px] text-[15px] leading-8" style={{ color: props.brand.muted }}>{body}</p> : null}<Link href={`${props.basePath}/all-products`} className="mt-8 inline-flex items-center gap-2 rounded-full px-6 py-3 text-[12px] font-black text-white no-underline" style={{ backgroundColor: props.brand.primary }}>Browse products<ArrowRight className="h-4 w-4" /></Link></div></section>
