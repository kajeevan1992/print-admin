...
export type StorefrontThemeSectionTypeSchema = { type: string; label: string; description?: string; defaults?: Record<string, unknown>; fields: StorefrontThemeSectionFieldSchema[]; };
...
export type StorefrontProductCard = { slug: string; category: string; title: string; text: string; image: string; price: string; priceFromMinor?: number; currency?: string; productType?: string; buyingMode?: 'cart' | 'quote'; optionGroups?: StorefrontProductOptionGroup[]; };
export type StorefrontCategoryCard = { slug: string; title: string; description: string; productCount: number; sortOrder: number; image: string; };
export type StorefrontCollectionPoint = { slug: string; name: string; address: string; note: string; status: string; };
export type StorefrontBrandSettings = { brandName: string; logoUrl: string; primary: string; accent: string; background: string; text: string; muted: string; border: string; };
export type StorefrontHomepageSection = Record<string, any> & { id: string; type: string; enabled: boolean; };
export type StorefrontContentPage = Record<string, any> & { id: string; path: string; title: string; summary: string; enabled: boolean; showInNavigation: boolean; navigationLabel: string; navigationOrder: number; seoTitle: string; seoDescription: string; socialImage: string; noIndex: boolean; sections: StorefrontHomepageSection[]; };
export type StorefrontRuntimeSettings = { tenantIds: string[]; storeSlug: string; storeName: string; storeStatus: string; storeFound?: boolean; themeKey: string; brand: StorefrontBrandSettings; content: Record<string, any>; layout: Record<string, any>; navigation: StorefrontMenuItem[]; navigationManaged?: boolean; sections: StorefrontHomepageSection[]; pages?: StorefrontContentPage[]; themePublished: boolean; themeVersion: number; source: 'store-and-published-theme' | 'store' | 'defaults'; };
...
export type StorefrontRuntimeContext = { tenantSlug: string; storeSlug: string; tenantIds: string[]; storeBase: string; routeSegments: string[]; searchParams?: StorefrontRuntimeSearchParams; themeKey: StorefrontThemeKey; themeSource: 'tenant-setting' | 'default'; themeManifest?: StorefrontThemeManifest; uploadedThemes?: StorefrontThemeManifest[]; navItems: StorefrontNavItem[]; products: StorefrontProductCard[]; categories?: StorefrontCategoryCard[]; collectionPoints?: StorefrontCollectionPoint[]; settings?: StorefrontRuntimeSettings; routeViews?: V0ThemeRouteViews; isDraftPreview?: boolean; };
