...
export default function StorefrontTestPage() {
  ...
  const [customerWorkloadReportRows, setCustomerWorkloadReportRows] = useState<any[]>([]);
  ...
  async function updateCustomerWorkloadPerformance(member: any, action: string) {
    ...
    if (json.ok) { await loadCustomerWorkloadPerformance(); await loadCustomerWorkloadReports(); await loadCustomerCommunications(); }
  }

  async function loadCustomerWorkloadReports() {
    try {
      const response = await fetch('/api/internal/catalog/customer-workload-reports', { cache: 'no-store' });
      const json = await response.json();
      setCustomerWorkloadReportRows(Array.isArray(json?.data?.rows) ? json.data.rows : []);
      setCustomerWorkloadReportSummary(json?.data?.summary || null);
      setCustomerWorkloadReports(Array.isArray(json?.data?.reports) ? json.data.reports : []);
      setCustomerWorkloadCsvPreview(String(json?.data?.csvPreview || ''));
    } catch (err) {
      setProductionStatus(err instanceof Error ? err.message : 'Could not load customer workload reports.');
    }
  }

  async function updateCustomerWorkloadReports(action: string) {
    setProductionStatus('Updating customer workload report...');
    const response = await fetch('/api/internal/catalog/customer-workload-reports', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action }) });
    const json = await response.json();
  ...
  async function loadPaymentRefunds() {
    try {
      const response = await fetch('/api/internal/catalog/payment-refunds', { cache: 'no-store' });
      const json = await response.json();
      setPaymentRefundItems(Array.isArray(json?.data?.items) ? json.data.items : []);
      setPaymentRefundActions(Array.isArray(json?.data?.actions) ? json.data.actions : []);
      setPaymentRefundSummary(json?.data?.summary || null);
    } catch (err) {
      setFinanceStatus(err instanceof Error ? err.message : 'Could not load payment refunds.');
    }
  }
  ...
  async function loadProductionJobRecords() {
    try {
      const response = await fetch('/api/internal/catalog/production-jobs', { cache: 'no-store' });
      const json = await response.json();
      setProductionJobItems(Array.isArray(json?.data?.items) ? json.data.items : []);
      setProductionJobActions(Array.isArray(json?.data?.actions) ? json.data.actions : []);
      setProductionJobSummary(json?.data?.summary || null);
    } catch (err) {
      setProductionStatus(err instanceof Error ? err.message : 'Could not load production job records.');
    }
  }

  async function updateProductionJobRecord(item: any, action: string) {
    const jobId = String(item?.id || '');
    setProductionStatus('Updating production job record...');
    const response = await fetch('/api/internal/catalog/production-jobs', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jobId, action, note: action === 'block' ? 'Blocked from storefront-test job board.' : undefined }),
    });
    const json = await response.json();
    setProductionStatus(json.ok ? `Production job updated: ${json.item?.status || action}` : (json.error || 'Production job update failed.'));
  ...
  async function splitOrderIntoProductionJobs() {
    ...
    const response = await fetch('/api/internal/catalog/production-jobs', {
      ...
      body: JSON.stringify({
        action: 'split-order',
        workflowId: source.id || source.workflowId,
        orderId: source.orderId || source.id,
        orderNumber: source.orderNumber,
        productName: source.productName || cartItems[0]?.productName || 'Storefront Product',
        quantity: source.quantity || cartItems[0]?.quantity || 1,
        preflightStatus: source.preflightStatus || 'pending',
      }),
    });
    const json = await response.json();
  ...
  async function updatePaymentRefund(item: any, action: string) {
    ...
    setFinanceStatus('Updating refund/reversal tracking...');
    const response = await fetch('/api/internal/catalog/payment-refunds', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ action, refundId, intentId, refundMinor: item?.refundMinor }),
    });
    const json = await response.json();
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6 text-white">
      ...
      <section className="rounded-3xl border border-fuchsia-500/20 bg-fuchsia-500/10 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-fuchsia-100/70">Production job records</p>
            <h2 className="mt-1 text-xl font-semibold text-fuchsia-50">v288 order split to production jobs</h2>
            <p className="mt-2 max-w-3xl text-sm text-fuchsia-100/75">Splits confirmed/preflighted orders into manufacturing job records such as print and finishing jobs. Jobs stay blocked until preflight is passed or overridden, so the production queue receives safe work only.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button className="rounded-xl border border-fuchsia-500/40 bg-fuchsia-500/10 px-3 py-2 text-sm font-medium text-fuchsia-100" onClick={splitOrderIntoProductionJobs}>Split Order</button>
            <button className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm font-medium text-textMuted" onClick={loadProductionJobRecords}>Refresh</button>
          </div>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-6">
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Jobs</p><p className="mt-1 font-semibold text-fuchsia-50">{Number(productionJobSummary?.total || 0)}</p></div>
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Blocked</p><p className="mt-1 font-semibold text-red-100">{Number(productionJobSummary?.blocked || 0)}</p></div>
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Ready</p><p className="mt-1 font-semibold text-emerald-100">{Number(productionJobSummary?.ready || 0)}</p></div>
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Queued</p><p className="mt-1 font-semibold text-fuchsia-50">{Number(productionJobSummary?.queued || 0)}</p></div>
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Print</p><p className="mt-1 font-semibold text-fuchsia-50">{Number(productionJobSummary?.print || 0)}</p></div>
          <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-fuchsia-100/70">Finishing</p><p className="mt-1 font-semibold text-fuchsia-50">{Number(productionJobSummary?.finishing || 0)}</p></div>
        </div>
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          {productionJobItems.slice(0, 8).map((job) => (
            <div key={String(job.id)} className="rounded-2xl border border-white/10 bg-black/10 p-4 text-xs text-fuchsia-100/80">
              <div className="flex items-start justify-between gap-3"><p className="font-medium text-fuchsia-50">{String(job.orderNumber || job.orderId || job.id)} - {String(job.jobType || '')}</p><span className={job.productionBlocked ? 'text-red-200' : 'text-emerald-200'}>{String(job.status || '')}</span></div>
              <p className="mt-1 text-fuchsia-100/70">{String(job.productName || '')} - qty {Number(job.quantity || 0)} - preflight {String(job.preflightStatus || '')}</p>
              <p className="mt-1 text-fuchsia-100/70">{String(job.materialKey || '')} to {String(job.machineKey || '')}</p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-100" onClick={() => updateProductionJobRecord(job, 'mark-preflight-pass')}>Pass</button>
                <button className="rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-1.5 text-xs font-medium text-red-100" onClick={() => updateProductionJobRecord(job, 'block')}>Block</button>
                <button className="rounded-lg border border-violet-500/40 bg-violet-500/10 px-3 py-1.5 text-xs font-medium text-violet-100 disabled:opacity-50" onClick={() => updateProductionJobRecord(job, 'release')} disabled={Boolean(job.productionBlocked && job.preflightStatus !== 'pass' && job.preflightStatus !== 'override')}>Release</button>
                <button className="rounded-lg border border-sky-500/40 bg-sky-500/10 px-3 py-1.5 text-xs font-medium text-sky-100" onClick={() => updateProductionJobRecord(job, 'start')}>Start</button>
                <button className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-textMuted" onClick={() => updateProductionJobRecord(job, 'complete')}>Done</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-3xl border border-border bg-panel p-5">
        ...
        {customerWorkloadReportSummary && (
          <div className="mt-5 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-sm font-medium text-emerald-100">Customer workload reporting</p><p className="mt-1 text-xs text-textMuted">Operational report snapshot and CSV preview for customer service workload, SLA pressure and team performance.</p></div>
              <div className="flex flex-wrap gap-2"><button className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-100" onClick={() => updateCustomerWorkloadReports('generate-summary')}>Generate Summary</button><button className="rounded-lg border border-cyan-500/40 bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-100" onClick={() => updateCustomerWorkloadReports('export-csv')}>Prepare CSV</button><button className="rounded-lg border border-white/20 bg-white/5 px-3 py-1 text-xs font-medium text-white" onClick={() => updateCustomerWorkloadReports('clear-reports')}>Clear Reports</button></div>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-6">
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">Team score</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.teamScore || 0)}</p></div>
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">Active</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.activeTasks || 0)}</p></div>
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">Urgent</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.urgentTasks || 0)}</p></div>
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">At risk</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.atRiskTasks || 0)}</p></div>
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">Breached</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.breachedTasks || 0)}</p></div>
              <div className="rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-textMuted">Saved</p><p className="text-xl font-semibold text-white">{Number(customerWorkloadReportSummary.reportsSaved || 0)}</p></div>
            </div>
            <div className="mt-4 grid gap-3 lg:grid-cols-4">
              {customerWorkloadReportRows.map((row) => (
                <div key={String(row.assigneeId)} className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-textMuted">
                  <div className="flex items-start justify-between gap-2"><div><p className="font-medium text-white">{String(row.name)}</p><p>{String(row.role || 'Team')}</p></div><span className="rounded-full border border-white/10 px-2 py-1 text-emerald-100">Score {Number(row.performanceScore || 0)}</span></div>
                  <div className="mt-3 space-y-1"><p>Capacity: {Number(row.capacity || 0)} · Active: {Number(row.activeTasks || 0)}</p><p>Completed: {Number(row.completedTasks || 0)} · Urgent: {Number(row.urgentTasks || 0)}</p><p>At risk: {Number(row.atRiskTasks || 0)} · Breached: {Number(row.breachedTasks || 0)}</p></div>
                </div>
              ))}
            </div>
            {customerWorkloadCsvPreview && <pre className="mt-4 max-h-48 overflow-auto rounded-xl border border-white/10 bg-black/30 p-3 text-xs text-emerald-50">{customerWorkloadCsvPreview}</pre>}
            {customerWorkloadReports.length > 0 && (<div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-textMuted"><p className="font-medium text-white">Saved workload reports</p><div className="mt-2 grid gap-2 lg:grid-cols-2">{customerWorkloadReports.slice(0, 4).map((report) => <p key={String(report.id)}>{String(report.createdAt)} · {String(report.reportType)} · score {Number(report.summary?.teamScore || 0)} · breached {Number(report.summary?.breachedTasks || 0)}</p>)}</div></div>)}
          </div>
        )}
      ...
      <section className="rounded-3xl border border-border bg-panel p-5">
        ...
        <div className="mt-4 rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-rose-50">Refund / reversal tracking</p>
              <p className="mt-1 text-xs text-rose-100/70">Track refund requests against captured payments and invoice status. Internal only — no live gateway money movement.</p>
            </div>
            <button className="rounded-lg border border-rose-500/40 bg-rose-500/10 px-3 py-1.5 text-xs font-medium text-rose-100" onClick={() => loadPaymentRefunds()}>Refresh refunds</button>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-4">
            <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-rose-100/70">Open value</p><p className="mt-1 font-semibold text-rose-50">{money(Number(paymentRefundSummary?.openMinor || 0), String(paymentRefundSummary?.currency || 'GBP'))}</p></div>
            <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-rose-100/70">Refunded</p><p className="mt-1 font-semibold text-rose-50">{money(Number(paymentRefundSummary?.refundedMinor || 0), String(paymentRefundSummary?.currency || 'GBP'))}</p></div>
            <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-rose-100/70">Requested</p><p className="mt-1 font-semibold text-rose-50">{Number(paymentRefundSummary?.requestedCount || 0)}</p></div>
            <div className="rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-rose-100/70">Actions</p><p className="mt-1 font-semibold text-rose-50">{Number(paymentRefundSummary?.actionCount || 0)}</p></div>
          </div>
          {paymentRefundItems.length === 0 ? (
            <p className="mt-3 rounded-xl border border-white/10 bg-black/10 p-3 text-xs text-rose-100/70">No captured payments are available for refund/reversal tracking yet.</p>
          ) : (
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              {paymentRefundItems.slice(0, 6).map((item) => (
                <div key={String(item.id)} className="rounded-xl border border-white/10 bg-black/10 p-3 text-xs text-rose-100/80">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="font-medium text-rose-50">{String(item.invoiceNumber || item.id)}</p><p className="mt-1">{String(item.status || 'available')} · {String(item.invoiceStatus || '')}</p></div>
                    <div className="text-right"><p className="font-semibold text-rose-50">{money(Number(item.refundMinor || 0), String(item.currency || 'GBP'))}</p><p className="mt-1">captured {money(Number(item.capturedMinor || 0), String(item.currency || 'GBP'))}</p></div>
                  </div>
                  <p className="mt-2 text-rose-100/70">{String(item.reason || 'Refund/reversal review')}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button className="rounded-lg border border-rose-500/40 bg-rose-500/10 px-3 py-1.5 text-xs font-medium text-rose-100" onClick={() => updatePaymentRefund(item, 'request-refund')}>Request</button>
                    <button className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-100" onClick={() => updatePaymentRefund(item, 'approve-refund')}>Approve</button>
                    <button className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-100" onClick={() => updatePaymentRefund(item, 'mark-refunded')}>Mark Refunded</button>
                    <button className="rounded-lg border border-sky-500/40 bg-sky-500/10 px-3 py-1.5 text-xs font-medium text-sky-100" onClick={() => updatePaymentRefund(item, 'reverse-payment')}>Reverse</button>
                    <button className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-textMuted" onClick={() => updatePaymentRefund(item, 'close-refund')}>Close</button>
                  </div>
                </div>
              ))}
            </div>
