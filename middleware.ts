...
const PROTECTED_PAGE_PREFIXES = ['/', '/workspace', '/super-admin', '/products', '/categories', '/orders', '/settings', '/tenant-control', '/shop-login-setup', '/fresh-db-setup', '/admin-users', '/api-keys', '/database-manager'];
const PUBLIC_PAGE_PREFIXES = ['/login', '/logout', '/theme', '/storefront', '/product', '/category', '/cart', '/checkout'];
...
function wantsHostedTheme(request: NextRequest) { const url = request.nextUrl; const host = request.headers.get('host') || ''; const mode = url.searchParams.get('hostedTheme') || request.headers.get('x-print-hosted-theme'); return mode === '1' || mode === 'true' || host.startsWith('theme.') || host.startsWith('store.'); }
...
function hostedThemeResponse(request: NextRequest, pathname: string, url: URL) { if (!wantsHostedTheme(request)) return null; if (RESERVED_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`))) return null; if (pathname === '/') { const rewriteUrl = url.clone(); rewriteUrl.pathname = '/theme/atlantis'; return NextResponse.rewrite(rewriteUrl); } if (pathname.startsWith('/category/') || pathname.startsWith('/product/') || pathname === '/cart' || pathname === '/checkout') return NextResponse.next(); const rewriteUrl = url.clone(); rewriteUrl.pathname = `/theme/atlantis${pathname === '/' ? '' : pathname}`; return NextResponse.rewrite(rewriteUrl); }
