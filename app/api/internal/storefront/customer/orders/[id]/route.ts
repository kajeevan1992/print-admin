...
function normalise(order: any, uploads: any[], jobs: any[]) { const artwork = artworkSummary(order, uploads); const production = productionSummary(order, jobs); const delivery = deliverySummary(order, production); return { ...order, created_at: order.createdAt, artwork_status: artwork.status, delivery: delivery.method, customer: { name: order.customerName, email: order.customerEmail }, artwork, production, deliveryStatus: delivery }; }
