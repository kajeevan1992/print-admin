...
function csv(rows: Array<Record<string, unknown>>) {
  ...
  return `\uFEFF${[headers.map(safeCell).join(','), ...rows.map((row) => headers.map((key) => safeCell(row[key])).join(','))].join('\r\n')}\r\n`;
}
...
function journalRows(report: FinanceReconciliationReport) {
  ...
  for (const invoice of report.invoices) {
    ...
    for (const note of invoice.creditNotes.filter((item) => item.status === 'issued')) {
...
function vatRows(report: FinanceReconciliationReport) {
  return report.vat.map((row) => ({ PeriodFrom: date(report.from), PeriodTo: date(report.to), VATRate: `${row.rate}%`, SalesNet: pounds(row.salesNetMinor), SalesVAT: pounds(row.salesVatMinor), SalesGross: pounds(row.salesGrossMinor), CreditNet: pounds(row.creditNetMinor), CreditVAT: pounds(row.creditVatMinor), CreditGross: pounds(row.creditGrossMinor), NetTaxableSales: pounds(row.netNetMinor), NetOutputVAT: pounds(row.netVatMinor), NetGrossSales: pounds(row.netGrossMinor), Currency: report.currency }));
}

function exceptionRows(report: FinanceReconciliationReport) {
  return report.issues.map((item) => ({ Severity: item.severity, Code: item.code, Message: item.message, OrderNumber: item.orderNumber, OrderId: item.orderId, InvoiceNumber: item.invoiceNumber, InvoiceId: item.invoiceId, Expected: pounds(item.expectedMinor), Actual: pounds(item.actualMinor), Currency: item.currency, PeriodFrom: date(report.from), PeriodTo: date(report.to) }));
}

export function buildAccountingExport(report: FinanceReconciliationReport, format: AccountingExportFormat) {
  const rows = format === 'journal' ? journalRows(report) : format === 'vat' ? vatRows(report) : format === 'exceptions' ? exceptionRows(report) : salesLedgerRows(report);
  const suffix = `${date(report.from)}_${date(report.to)}`;
  return { filename: `${format}_${suffix}.csv`, contentType: 'text/csv; charset=utf-8', body: csv(rows), rowCount: rows.length };
}
