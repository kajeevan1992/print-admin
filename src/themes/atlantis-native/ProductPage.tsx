...
function productAction(product: ThemeProductCard, storeBase: string, category: string, slug: string) {
  const isQuote = product.buyingMode === 'quote';
  return {
    label: isQuote ? 'Request quote' : 'Add to cart',
    href: isQuote ? `${storeBase}/quote/${category}/${slug}` : `${storeBase}/cart?product=${encodeURIComponent(slug)}&category=${encodeURIComponent(category)}`,
