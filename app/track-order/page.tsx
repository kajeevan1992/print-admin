...
import { AlertTriangle, CheckCircle2, Clock3, CreditCard, ExternalLink, MapPinned, PackageCheck, RefreshCcw, Search, ShieldCheck, Truck, UploadCloud } from 'lucide-react';
...
function eventTone(action: string) { if (action.includes('approved') || action.includes('delivered') || action.includes('collected')) return 'border-emerald-400/25 bg-emerald-400/10 text-emerald-100'; if (action.includes('revision') || action.includes('exception')) return 'border-amber-400/25 bg-amber-400/10 text-amber-100'; if (action.includes('sent') || action.includes('dispatch') || action.includes('transit')) return 'border-sky-400/25 bg-sky-400/10 text-sky-100'; return 'border-white/10 bg-white/[0.04] text-slate-300'; }
...
function ShipmentCard({ shipment }: { shipment: any }) {
  ...
  return <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
    ...
    <div className="mt-3 space-y-2 text-sm text-slate-400">
      ...
      {shipment.packageCount ? <p>{shipment.packageCount} package(s)</p> : null}
...
export default function TrackOrderPage() {
  ...
  async function lookup(id = orderId, mail = email) {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Order status failed.'); }
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    {status ? <>
      ...
      <TimelineCard title="Shipment timeline" subtitle="Packing, handover, collection and delivery events recorded by the print team." events={status.shipmentEvents || status.dispatch?.events} />
