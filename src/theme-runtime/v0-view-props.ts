...
export function buildV0ThemeNavigation(basePath: string, navItems: NavItem[], currentPath: string): V0ThemeNavigationItem[] {
  const navigation = navItems.map((item) => ({ label: item.label, href: href(basePath, item.path), active: currentPath === item.path || currentPath.startsWith(`${item.path}/`) }));
  if (!navigation.some((item) => item.href === href(basePath, '/search'))) navigation.push({ label: 'Search', href: href(basePath, '/search'), active: currentPath === '/search' });
  return navigation;
}
