...
export async function GET(request: Request) {
  ...
  const script = [
    ...
    'function n(r,i){r=r||{};var label=r.label||r.name||r.title||r.path||("Menu "+(i+1));return Object.assign({},r,{id:String(r.id||r.slug||label),slug:c(r.slug||label),label:label,path:p(r.path||r.href||r.url||"/"),enabled:r.enabled!==false,order:Number(r.order||r.sortOrder||i+1),parentId:String(r.parentId||""),parentSlug:c(r.parentSlug||""),children:Array.isArray(r.children)?r.children.map(n).filter(function(x){return x.enabled;}):[]});}',
    ...
    'function patch(){if(!loaded||!items.length)return;var nav=document.querySelector("header nav");if(!nav)return;var sp=split();if(!sp.top.length)return;var sig=sp.top.map(function(x){return x.id+":"+x.label+":"+x.path;}).join("|")+"|"+items.length;if(nav.getAttribute("data-pa-menu")===sig)return;var t=nav.querySelector("button,a");nav.setAttribute("data-pa-menu",sig);nav.innerHTML="";sp.top.slice(0,10).forEach(function(x){var b=document.createElement("button");b.type="button";b.className=t&&t.className?t.className:"inline-flex items-center gap-1 text-[13px] font-semibold tracking-[-0.01em]";b.style.cssText=t&&t.style?t.style.cssText:"";var has=kids(x,sp.map).length>0;b.textContent=x.label+(has?"⌄":"");b.onclick=function(e){e.preventDefault();go(x.path);};if(has){b.onmouseenter=function(){dd(x);};b.onfocus=function(){dd(x);};}nav.appendChild(b);});}',
    ...
    'load();setInterval(patch,500);document.addEventListener("click",function(e){var d=document.getElementById("pa-menu-dd");if(d&&!d.contains(e.target)&&!e.target.closest("header nav"))close();});',
    '})();',
  ].join('\n');
