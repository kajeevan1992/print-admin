...
import { buildPricingCostBreakdown } from '../../../../../src/core/catalog/pricing-cost-breakdown';
...
export async function GET(request: Request) {
  try {
    ...
    const productionEstimate = estimatePrintProduction(quoteInput, quantity);
    const costBreakdown = buildPricingCostBreakdown(quoteInput, productionEstimate, quantity, product);
    return NextResponse.json({ ok: true, source: 'internal-pricing-cost-breakdown', data: { quoteInput, productionEstimate, costBreakdown } });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Pricing cost breakdown failed.';
    return NextResponse.json({ ok: false, source: 'internal-pricing-cost-breakdown', error: message }, { status: 500 });
  }
}
