...
import { FileText, LogOut, MapPin, Package, Palette, Receipt, RefreshCw, UserRound } from 'lucide-react';
...
export default function CustomerAccountClient({ mode, section = 'overview', tenantSlug, storeSlug, storeBase, returnUrl, customer, orders: suppliedOrders = [], addresses: suppliedAddresses = [], summary }: { mode: 'login' | 'register' | 'dashboard'; section?: Section; tenantSlug: string; storeSlug: string; storeBase: string; returnUrl?: string; customer?: { id: string; name: string; email: string; phone: string; company: string } | null; orders?: AccountOrder[]; addresses?: Address[]; summary?: Summary }) {
  const [busy, setBusy] = useState('');
  const [error, setError] = useState('');
  const [addresses, setAddresses] = useState(suppliedAddresses);
  const orders = suppliedOrders;
  const quotes = useMemo(() => orders.filter((order) => order.status === 'AWAITING_APPROVAL' || order.quoteReference), [orders]);
  const invoices = useMemo(() => orders.filter((order) => ['paid', 'authorized', 'refunded'].includes(clean(order.paymentStatus).toLowerCase())), [orders]);
  ...
  async function logout() {
    setBusy('logout');
    try { const payload = await post({ action: 'logout' }); window.location.assign(payload.redirectUrl || storeBase); }
    catch (next) { setError(next instanceof Error ? next.message : 'Sign-out failed.'); setBusy(''); }
  }

  async function saveAddress(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy('address');
    try { const data = Object.fromEntries(new FormData(event.currentTarget).entries()); const payload = await post({ action: 'save-address', ...data }); setAddresses(Array.isArray(payload.addresses) ? payload.addresses : addresses); event.currentTarget.reset(); }
    catch (next) { setError(next instanceof Error ? next.message : 'Address could not be saved.'); }
    finally { setBusy(''); }
  }

  async function removeAddress(id: string) {
    setBusy(`delete:${id}`);
    try { const payload = await post({ action: 'delete-address', id }); setAddresses(Array.isArray(payload.addresses) ? payload.addresses : addresses.filter((address) => address.id !== id)); }
    catch (next) { setError(next instanceof Error ? next.message : 'Address could not be removed.'); }
    finally { setBusy(''); }
  }

  async function repeatOrder(orderId: string) {
    setBusy(`repeat:${orderId}`);
    try { const payload = await post({ action: 'repeat-order', orderId }); window.location.assign(payload.redirectUrl || `${storeBase}/cart`); }
    catch (next) { setError(next instanceof Error ? next.message : 'This order could not be repeated.'); setBusy(''); }
  }
  ...
  return <div className="grid gap-6 lg:grid-cols-[250px_1fr]">
    <aside className="h-fit rounded-[24px] border bg-white p-4" style={{ borderColor: 'var(--storefront-line, #E3E8F0)' }}><div className="p-3"><div className="text-xs font-black uppercase tracking-[0.16em]" style={{ color: 'var(--storefront-primary, #18A7D0)' }}>Customer account</div><div className="mt-2 font-black">{customer.name}</div><div className="mt-1 truncate text-xs text-slate-500">{customer.email}</div></div><nav className="mt-3 space-y-1">{nav.map(([key, label, Icon]) => <Link key={key} href={`${storeBase}/account${key === 'overview' ? '' : `/${key}`}`} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold no-underline" style={{ backgroundColor: section === key ? 'color-mix(in srgb, var(--storefront-primary, #18A7D0) 10%, white)' : 'transparent', color: section === key ? 'var(--storefront-primary, #18A7D0)' : 'var(--storefront-ink, #111827)' }}><Icon className="h-4 w-4" />{label}</Link>)}</nav><button onClick={logout} disabled={busy === 'logout'} className="mt-4 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-red-700"><LogOut className="h-4 w-4" />Sign out</button></aside>
    <section>{error ? <div className="mb-5 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">{error}</div> : null}
      {section === 'overview' ? <><div><div className="text-[11px] font-black uppercase tracking-[0.18em]" style={{ color: 'var(--storefront-primary, #18A7D0)' }}>Account overview</div><h1 className="mt-3 text-[38px] font-black tracking-[-0.055em]">Hello, {customer.name.split(' ')[0]}</h1><p className="mt-2 text-sm text-slate-500">Track print jobs, quotes, artwork, invoice records and saved delivery details.</p></div><div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => <Link key={card.label} href={card.href} className="rounded-[20px] border bg-white p-5 no-underline" style={{ borderColor: 'var(--storefront-line, #E3E8F0)', color: 'var(--storefront-ink, #111827)' }}><div className="text-3xl font-black">{card.value}</div><div className="mt-2 text-xs font-bold text-slate-500">{card.label}</div></Link>)}</div><div className="mt-8"><h2 className="mb-4 text-2xl font-black">Recent orders</h2><OrderList values={orders.slice(0, 5)} empty="No orders have been placed with this email yet." /></div></> : null}
      ...
      {section === 'quotes' ? <><h1 className="mb-6 text-[38px] font-black tracking-[-0.055em]">Quotes</h1><OrderList values={quotes} empty="Formal and approval-led quote records will appear here." /></> : null}
      {section === 'invoices' ? <><h1 className="text-[38px] font-black tracking-[-0.055em]">Invoices</h1><p className="mb-6 mt-2 text-sm text-slate-500">Paid-order invoice records are shown here. Downloadable VAT invoice PDFs will be added in the formal invoicing phase.</p><OrderList values={invoices} empty="Paid order invoice records will appear here." /></> : null}
