...
function internalPaymentToAdmin(value: unknown): PaymentStatus { const raw = String(value || '').toLowerCase(); if (raw.includes('refund')) return 'refunded'; if (raw.includes('failed')) return 'failed'; if (raw.includes('paid')) return 'paid'; if (raw.includes('author')) return 'authorized'; return 'unpaid'; }
...
function adminPaymentToInternal(status: PaymentStatus) { const map: Record<PaymentStatus, string> = { unpaid: 'AWAITING_PAYMENT', authorized: 'AWAITING_PAYMENT', paid: 'APPROVED', failed: 'AWAITING_PAYMENT', refunded: 'CANCELLED' }; return map[status] || 'AWAITING_PAYMENT'; }
...
export const ordersService = {
  ...
  adminPaymentAction: async (id: string, body: { action: 'mark-paid' | 'mark-failed' | 'mark-refunded' | 'refund-note'; note?: string; reference?: string; amountMinor?: number; actor?: string; paymentProvider?: string }): Promise<ApiResponse<Order>> => { const patched = await paymentAction(id, body); if (patched) return ok(patched); const current = readStore().find((order) => order.id === id) ?? ordersStore[0]; const paymentStatus = body.action === 'mark-paid' ? 'paid' : body.action === 'mark-failed' ? 'failed' : body.action === 'mark-refunded' ? 'refunded' : current.paymentStatus; return ordersService.saveOrder({ ...current, paymentStatus, refundNote: body.note || current.refundNote, updatedAt: new Date().toISOString() }); },
