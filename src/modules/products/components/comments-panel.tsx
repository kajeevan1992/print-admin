...
export function CommentsPanel({ comments }: { comments: ProductComment[] }) {
  ...
  return (
    <ProductSectionCard title="Comments & Notes">
      <div className="space-y-3">
        {comments.length === 0 ? <p className="text-sm text-textMuted">No comments yet.</p> : comments.map((comment) => (
          <div key={comment.id} className="rounded-lg border border-border bg-panelMuted p-3 text-sm">
            ...
            <p className="mb-1 text-xs uppercase text-accentAlt">{comment.label}</p>
