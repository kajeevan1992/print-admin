...
import { listOrderRecords, readOrderRecord, saveOrderRecord } from './order-records.service';
...
export async function saveOrder(request: Request, input: OrderInput) {
  ...
  const existingRow = orderId ? await readOrderRecord(tenantId, orderId) : await readOrderRecord(tenantId, orderNumber);
  ...
  const order = await saveOrderRecord({ id: existingRow?.id || orderId || undefined, tenantId, customerId: user?.id || null, orderNumber, status: statusFrom(input), currency: totals.currency, subtotalMinor: totals.subtotalMinor, shippingMinor: totals.shippingMinor, taxMinor: totals.taxMinor, totalMinor: totals.totalMinor, notes, items });
  if (!order) throw new Error('Order could not be saved.');
  return normalize(order);
}
export async function getOrder(request: Request, id: string) { const tenantId = await tenantIdFromRequest(request); const order = await readOrderRecord(tenantId, id); return order ? normalize(order) : null; }
export async function listOrders(request: Request, options: { email?: string | null; status?: string | null; limit?: number; search?: string | null } = {}) { const tenantId = await tenantIdFromRequest(request); const orders = await listOrderRecords(tenantId, { status: options.status ? statusFrom({ status: options.status }) : null, limit: Math.max(1, Math.min(100, Number(options.limit || 50))) }); let normalized = orders.map(normalize); if (options.email) normalized = normalized.filter((order) => order.customerEmail.toLowerCase() === String(options.email).toLowerCase()); if (options.search) { const term = String(options.search).toLowerCase(); normalized = normalized.filter((order) => [order.orderNumber, order.customerName, order.customerEmail, order.customerCompany].join(' ').toLowerCase().includes(term)); } return normalized; }
