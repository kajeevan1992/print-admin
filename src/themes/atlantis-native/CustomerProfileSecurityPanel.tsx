...
import { CheckCircle2, KeyRound, Laptop, LogOut, MailCheck, ShieldCheck, Smartphone, UserRound, XCircle } from 'lucide-react';
...
type EmailChange = { id: string; oldEmail: string; newEmail: string; oldConfirmed: boolean; newConfirmed: boolean; expiresAt: string; createdAt: string };
...
export default function CustomerProfileSecurityPanel({ tenantSlug, storeSlug, storeBase, initialCustomer }: { tenantSlug: string; storeSlug: string; storeBase: string; initialCustomer: Customer }) {
  ...
  async function loadAccount() {
    ...
    try {
      ...
      setEmailChange(payload.emailChange || null);
    } catch (next) { setError(next instanceof Error ? next.message : 'Account security details could not be loaded.'); }
  ...
  async function post(body: Record<string, unknown>) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'emailChange')) setEmailChange(payload.emailChange || null);
  ...
  async function requestEmailChange(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setBusy('email-change'); setNotice('');
    try { const form = event.currentTarget; const data = Object.fromEntries(new FormData(form).entries()); await post({ action: 'request-email-change', ...data }); form.reset(); }
    catch (next) { setError(next instanceof Error ? next.message : 'The email-change request could not be started.'); }
    finally { setBusy(''); }
  }

  async function cancelEmailChange() {
    if (!emailChange) return;
    setBusy('cancel-email-change'); setNotice('');
    try { await post({ action: 'cancel-email-change', changeId: emailChange.id }); }
    catch (next) { setError(next instanceof Error ? next.message : 'The pending email change could not be cancelled.'); }
    finally { setBusy(''); }
  }
  ...
  return <div>
    ...
    <p className="mt-2 max-w-[760px] text-sm leading-7 text-slate-500">Update checkout details, securely change the verified login email, rotate your password and review every active customer session for this store.</p>
    ...
    <section className="mt-8 rounded-[24px] border bg-white p-6" style={{ borderColor: 'var(--storefront-line, #E3E8F0)' }}>
      <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-slate-100"><MailCheck className="h-5 w-5" /></div><div><h2 className="text-xl font-black">Change login email</h2><p className="text-xs text-slate-500">Protected by your password and confirmation from both addresses.</p></div></div>
      {emailChange ? <div className="mt-6 rounded-[20px] border bg-slate-50 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Pending replacement</div><div className="mt-2 break-all text-lg font-black">{emailChange.newEmail}</div><div className="mt-2 text-xs leading-6 text-slate-500">Expires {dateTime(emailChange.expiresAt)}. Nothing changes until both addresses confirm.</div></div><button type="button" onClick={cancelEmailChange} disabled={busy === 'cancel-email-change'} className="inline-flex items-center justify-center gap-2 text-xs font-black text-red-700 disabled:opacity-50"><XCircle className="h-4 w-4" />{busy === 'cancel-email-change' ? 'Cancelling…' : 'Cancel request'}</button></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className={`rounded-xl border p-4 ${emailChange.oldConfirmed ? 'border-emerald-200 bg-emerald-50' : 'bg-white'}`}><div className="text-xs font-black">Current email approval</div><div className="mt-1 truncate text-xs text-slate-500">{emailChange.oldEmail}</div><div className={`mt-2 text-[10px] font-black uppercase ${emailChange.oldConfirmed ? 'text-emerald-700' : 'text-amber-700'}`}>{emailChange.oldConfirmed ? 'Confirmed' : 'Waiting for link'}</div></div><div className={`rounded-xl border p-4 ${emailChange.newConfirmed ? 'border-emerald-200 bg-emerald-50' : 'bg-white'}`}><div className="text-xs font-black">New email verification</div><div className="mt-1 truncate text-xs text-slate-500">{emailChange.newEmail}</div><div className={`mt-2 text-[10px] font-black uppercase ${emailChange.newConfirmed ? 'text-emerald-700' : 'text-amber-700'}`}>{emailChange.newConfirmed ? 'Confirmed' : 'Waiting for link'}</div></div></div>
      </div> : <form onSubmit={requestEmailChange} className="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end"><label className="grid gap-1.5 text-xs font-bold text-slate-600">New email address<input required name="newEmail" type="email" autoComplete="email" placeholder="new@email.com" className="rounded-xl border px-4 py-3 text-sm font-normal text-slate-900" /></label><label className="grid gap-1.5 text-xs font-bold text-slate-600">Current password<input required name="currentPassword" type="password" autoComplete="current-password" placeholder="Confirm your password" className="rounded-xl border px-4 py-3 text-sm font-normal text-slate-900" /></label><button disabled={busy === 'email-change'} className="rounded-full px-5 py-3 text-sm font-black text-white disabled:opacity-50" style={{ backgroundColor: 'var(--storefront-primary, #18A7D0)' }}>{busy === 'email-change' ? 'Sending links…' : 'Start secure change'}</button></form>}
      <div className="mt-4 flex items-start gap-2 rounded-xl bg-slate-50 p-3 text-xs leading-6 text-slate-600"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />After both links are approved, the new address becomes verified automatically and every customer session is signed out.</div>
    </section>
